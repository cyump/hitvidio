  ...
  sources_common(this, sorter)
{
  ...
  toolbar_widget->setVisible(hitvidio::CurrentConfig.show_project_toolbar);
  ...
  toolbar_new->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/add-button.svg")));
  ...
  toolbar_open->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/open.svg")));
  toolbar_open->setToolTip(tr("Open Project"));
  connect(toolbar_open, SIGNAL(clicked(bool)), hitvidio::Global.get(), SLOT(OpenProject()));
  toolbar->addWidget(toolbar_open);

  QPushButton* toolbar_save = new QPushButton();
  toolbar_save->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/save.svg")));
  toolbar_save->setToolTip(tr("Save Project"));
  connect(toolbar_save, SIGNAL(clicked(bool)), hitvidio::Global.get(), SLOT(save_project()));
  toolbar->addWidget(toolbar_save);

  QPushButton* toolbar_undo = new QPushButton();
  toolbar_undo->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/undo.svg")));
  toolbar_undo->setToolTip(tr("Undo"));
  connect(toolbar_undo, SIGNAL(clicked(bool)), hitvidio::Global.get(), SLOT(undo()));
  toolbar->addWidget(toolbar_undo);

  QPushButton* toolbar_redo = new QPushButton();
  toolbar_redo->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/redo.svg")));
  toolbar_redo->setToolTip(tr("Redo"));
  connect(toolbar_redo, SIGNAL(clicked(bool)), hitvidio::Global.get(), SLOT(redo()));
  ...
  toolbar_tree_view->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/treeview.svg")));
  ...
  toolbar_icon_view->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/iconview.svg")));
  ...
  toolbar_list_view->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/listview.svg")));
  ...
  directory_up->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/dirup.svg")));
  ...
  connect(hitvidio::media_icon_service.get(), SIGNAL(IconChanged()), icon_view->viewport(), SLOT(update()));
  connect(hitvidio::media_icon_service.get(), SIGNAL(IconChanged()), tree_view->viewport(), SLOT(update()));
...
{
  sorter.setSourceModel(&hitvidio::project_model);
}
...
QString Project::get_next_sequence_name(QString start) {
  ...
  while (found) {
    ...
    for (int i=0;i<hitvidio::project_model.childCount();i++) {
      if (QString::compare(hitvidio::project_model.child(i)->get_name(), name, Qt::CaseInsensitive) == 0) {
        found = true;
        n++;
        break;
      }
...
SequencePtr create_sequence_from_media(QVector<hitvidio::timeline::MediaImportData>& media_list) {
  SequencePtr s(new Sequence());

  s->name = panel_project->get_next_sequence_name();

  // Retrieve default Sequence settings from Config
  s->width = hitvidio::CurrentConfig.default_sequence_width;
  s->height = hitvidio::CurrentConfig.default_sequence_height;
  s->frame_rate = hitvidio::CurrentConfig.default_sequence_framerate;
  s->audio_frequency = hitvidio::CurrentConfig.default_sequence_audio_frequency;
  s->audio_layout = hitvidio::CurrentConfig.default_sequence_audio_channel_layout;
...
void Project::replace_clip_media() {
  if (hitvidio::ActiveSequence == nullptr) {
    ...
    if (selected_items.size() == 1) {
      ...
      if (item->get_type() == MEDIA_TYPE_SEQUENCE && hitvidio::ActiveSequence == item->to_sequence()) {
        QMessageBox::critical(this,
...
void Project::new_folder() {
  ...
  hitvidio::UndoStack.push(new AddMediaCommand(m, get_selected_folder()));

  QModelIndex index = hitvidio::project_model.create_index(m->row(), 0, m.get());
  switch (hitvidio::CurrentConfig.project_view_type) {
  case hitvidio::PROJECT_VIEW_TREE:
    tree_view->edit(sorter.mapFromSource(index));
    break;
  case hitvidio::PROJECT_VIEW_ICON:
...
MediaPtr Project::create_sequence_internal(ComboAction *ca, SequencePtr s, bool open, Media* parent) {
  ...
  if (ca != nullptr) {
    ...
    hitvidio::project_model.appendChild(parent, item);

    if (open) {
      hitvidio::Global->set_sequence(s);
    }
...
void Project::delete_selected_media() {
  ...
  if (remove) {
    ...
    if (hitvidio::ActiveSequence != nullptr) hitvidio::ActiveSequence->selections.clear();
    ...
    for (int i=0;i<items.size();i++) {
      ...
      if (items.at(i)->get_type() == MEDIA_TYPE_SEQUENCE) {
        ...
        if (s == hitvidio::ActiveSequence.get()) {
...
void Project::process_file_list(QStringList& files, bool recursive, MediaPtr replace, Media* parent) {
  ...
  QStringList image_sequence_formats = hitvidio::CurrentConfig.img_seq_formats.split("|");
  ...
  for (int i=0;i<files.size();i++) {
    ...
    if (QFileInfo(files.at(i)).isDir()) {
      ...
      if (create_undo_action) {
        ...
        hitvidio::project_model.appendChild(parent, folder);
      }
      ...
      // Check if the user is importing an Hitvidio project file
      if (file.endsWith(".ove", Qt::CaseInsensitive)) {

        // This file is an Hitvidio project file. Ask the user if they really want to import it.
        if (QMessageBox::question(this,
                                  tr("Import a Project"),
                                  tr("\"%1\" is an Hitvidio project file. It will merge with this project. "
                                     "Do you wish to continue?").arg(file),
                                  QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes) {

          // load the project without clearing the current one
          hitvidio::Global->ImportProject(file);

        }

      } else {

        // This file is NOT an Hitvidio project file
...
bool Project::reveal_media(Media *media, QModelIndex parent) {
  for (int i=0;i<hitvidio::project_model.rowCount(parent);i++) {
    const QModelIndex& item = hitvidio::project_model.index(i, 0, parent);
    Media* m = hitvidio::project_model.getItem(item);
    ...
    if (m->get_type() == MEDIA_TYPE_FOLDER) {
      ...
      if (hitvidio::CurrentConfig.project_view_type == hitvidio::PROJECT_VIEW_TREE) {
      ...
      } else if (hitvidio::CurrentConfig.project_view_type == hitvidio::PROJECT_VIEW_ICON) {
...
void Project::delete_clips_using_selected_media() {
  if (hitvidio::ActiveSequence == nullptr) {
    ...
    for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
      const ClipPtr& c = hitvidio::ActiveSequence->clips.at(i);
      if (c != nullptr) {
        for (int j=0;j<items.size();j++) {
          Media* m = item_to_media(items.at(j));
          if (c->media() == m) {
            ca->append(new DeleteClipAction(hitvidio::ActiveSequence.get(), i));
            deleted = true;
          }
        }
      }
      ...
      if (m->get_type() == MEDIA_TYPE_FOLDER) {
        if (set_ids_only) {
          ...
          if (!item.parent().isValid()) {
            ...
            stream.writeAttribute("parent", QString::number(hitvidio::project_model.getItem(item.parent())->temp_id));
          }
          stream.writeEndElement();
        }
...
void Project::save_project(bool autorecovery) {
  ...
  QFile file(autorecovery ? autorecovery_filename : hitvidio::ActiveProjectFilename);
  ...
  stream.writeTextElement("version", QString::number(hitvidio::kSaveVersion));

  stream.writeTextElement("url", hitvidio::ActiveProjectFilename);
  proj_dir = QFileInfo(hitvidio::ActiveProjectFilename).absoluteDir();
  ...
  if (!autorecovery) {
    add_recent_project(hitvidio::ActiveProjectFilename);
    hitvidio::Global->set_modified(false);
  }
}

void Project::update_view_type() {
  tree_view->setVisible(hitvidio::CurrentConfig.project_view_type == hitvidio::PROJECT_VIEW_TREE);
  icon_view_container->setVisible(hitvidio::CurrentConfig.project_view_type == hitvidio::PROJECT_VIEW_ICON
                                  || hitvidio::CurrentConfig.project_view_type == hitvidio::PROJECT_VIEW_LIST);
  ...
  case hitvidio::PROJECT_VIEW_TREE:
    ...
    break;
  case hitvidio::PROJECT_VIEW_ICON:
  case hitvidio::PROJECT_VIEW_LIST:
    icon_view->setViewMode(hitvidio::CurrentConfig.project_view_type == hitvidio::PROJECT_VIEW_ICON ?
...
void Project::set_icon_view() {
  hitvidio::CurrentConfig.project_view_type = hitvidio::PROJECT_VIEW_ICON;
  update_view_type();
}
...
void Project::set_tree_view() {
  hitvidio::CurrentConfig.project_view_type = hitvidio::PROJECT_VIEW_TREE;
  update_view_type();
}
...
void Project::list_all_sequences_worker(QVector<Media*>* list, Media* parent) {
  for (int i=0;i<hitvidio::project_model.childCount(parent);i++) {
    Media* item = hitvidio::project_model.child(i, parent);
    switch (item->get_type()) {
    case MEDIA_TYPE_SEQUENCE:
      list->append(item);
      break;
