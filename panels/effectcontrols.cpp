...
void EffectControls::set_zoom(bool in) {
  zoom *= (in) ? 2 : 0.5;
  update_keyframes();
  scroll_to_frame(hitvidio::ActiveSequence->playhead);
}
...
void EffectControls::setup_ui() {
  ...
  QIcon add_effect_icon = hitvidio::icon::CreateIconFromSVG(":/icons/add-effect.svg", false);
  QIcon add_transition_icon = hitvidio::icon::CreateIconFromSVG(":/icons/add-transition.svg", false);
...
void EffectControls::effects_area_context_menu() {
  ...
  hitvidio::MenuHelper.create_effect_paste_action(&menu);
...
void EffectControls::SetClips()
{
  ...
  if (hitvidio::ActiveSequence == nullptr) {
    ...
    // replace clip vector
    selected_clips_ = hitvidio::ActiveSequence->SelectedClips(false);
