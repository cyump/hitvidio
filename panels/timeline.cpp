...
int hitvidio::timeline::kTrackDefaultHeight = 40;
int hitvidio::timeline::kTrackMinHeight = 30;
int hitvidio::timeline::kTrackHeightIncrement = 10;
...
void Timeline::split_clip_at_positions(ComboAction* ca, int clip_index, QVector<long> positions) {
  ...
  Clip* clip = hitvidio::ActiveSequence->clips.at(clip_index).get();
  ...
  for (int i=0;i<post_splits.size();i++) {
    ...
    ca->append(new AddClipCommand(hitvidio::ActiveSequence.get(), post_splits[i]));
  }
...
void Timeline::previous_cut() {
  if (hitvidio::ActiveSequence != nullptr
      && hitvidio::ActiveSequence->playhead > 0) {
    long p_cut = 0;
    for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
      ClipPtr c = hitvidio::ActiveSequence->clips.at(i);
      if (c != nullptr) {
        if (c->timeline_out() > p_cut && c->timeline_out() < hitvidio::ActiveSequence->playhead) {
          p_cut = c->timeline_out();
        } else if (c->timeline_in() > p_cut && c->timeline_in() < hitvidio::ActiveSequence->playhead) {
          p_cut = c->timeline_in();
        }
      }
    }
...
void Timeline::next_cut() {
  if (hitvidio::ActiveSequence != nullptr) {
    ...
    for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
      ClipPtr c = hitvidio::ActiveSequence->clips.at(i);
      if (c != nullptr) {
        if (c->timeline_in() < n_cut && c->timeline_in() > hitvidio::ActiveSequence->playhead) {
          n_cut = c->timeline_in();
          seek_enabled = true;
        } else if (c->timeline_out() < n_cut && c->timeline_out() > hitvidio::ActiveSequence->playhead) {
...
void Timeline::toggle_show_all() {
  if (hitvidio::ActiveSequence != nullptr) {
    ...
    if (showing_all) {
      old_zoom = zoom;
      set_zoom_value(double(timeline_area->width() - 200) / double(hitvidio::ActiveSequence->getEndFrame()));
    } else {
...
void Timeline::create_ghosts_from_media(Sequence* seq, long entry_point, QVector<hitvidio::timeline::MediaImportData>& media_list) {
  video_ghosts = false;
  audio_ghosts = false;
  ...
  for (int i=0;i<media_list.size();i++) {
    ...
    const hitvidio::timeline::MediaImportData import_data = media_list.at(i);
    ...
    if (can_import) {
      ...
      case MEDIA_TYPE_FOOTAGE:
        ...
        if (import_data.type() == hitvidio::timeline::kImportAudioOnly
            || import_data.type() == hitvidio::timeline::kImportBoth) {
        ...
        if (import_data.type() == hitvidio::timeline::kImportVideoOnly
            || import_data.type() == hitvidio::timeline::kImportBoth) {
...
void Timeline::add_clips_from_ghosts(ComboAction* ca, Sequence* s) {
  ...
  for (int i=0;i<added_clips.size();i++) {
    ...
    if (hitvidio::CurrentConfig.add_default_effects_to_clips) {
      if (c->track() < 0) {
  ...
  if (hitvidio::CurrentConfig.enable_seek_to_import) {
...
void Timeline::add_transition() {
  ...
  for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
    Clip* c = hitvidio::ActiveSequence->clips.at(i).get();
    if (c != nullptr && c->IsSelected()) {
      ...
      if (c->opening_transition == nullptr) {
        ca->append(new AddTransitionCommand(c,
                                            ...
                                            hitvidio::CurrentConfig.default_transition_length));
        adding = true;
      }
  ...
  if (hitvidio::ActiveSequence != nullptr) {
    ...
    if (!selected_clips.isEmpty()) {
      ...
      for (int i=0;i<selected_clips.size();i++) {
        earliest_point = qMin(hitvidio::ActiveSequence->clips.at(selected_clips.at(i))->timeline_in(), earliest_point);
      }
      ...
      s->width = hitvidio::ActiveSequence->width;
      s->height = hitvidio::ActiveSequence->height;
      s->frame_rate = hitvidio::ActiveSequence->frame_rate;
      s->audio_frequency = hitvidio::ActiveSequence->audio_frequency;
      s->audio_layout = hitvidio::ActiveSequence->audio_layout;

      // copy all selected clips to the nest
      for (int i=0;i<selected_clips.size();i++) {
        // delete clip from old sequence
        ca->append(new DeleteClipAction(hitvidio::ActiveSequence.get(), selected_clips.at(i)));

        // copy to new
        ClipPtr copy(hitvidio::ActiveSequence->clips.at(selected_clips.at(i))->copy(s.get()));
      ...
      QVector<hitvidio::timeline::MediaImportData> media_list;
      media_list.append(m.get());
      create_ghosts_from_media(hitvidio::ActiveSequence.get(), earliest_point, media_list);

      // ensure ghosts won't overlap anything
      for (int j=0;j<hitvidio::ActiveSequence->clips.size();j++) {
        Clip* c = hitvidio::ActiveSequence->clips.at(j).get();
      ...
      add_clips_from_ghosts(ca, hitvidio::ActiveSequence.get());

      panel_graph_editor->set_row(nullptr);
      panel_effect_controls->Clear(true);
      hitvidio::ActiveSequence->selections.clear();
...
void Timeline::update_sequence() {
  bool null_sequence = (hitvidio::ActiveSequence == nullptr);
...
bool Timeline::focused() {
  return (hitvidio::ActiveSequence != nullptr && (headers->hasFocus() || video_area->hasFocus() || audio_area->hasFocus()));
}
...
void Timeline::repaint_timeline() {
  if (!block_repaints) {
    ...
    if (hitvidio::ActiveSequence != nullptr
        ...
        && !zoom_just_changed) {
      // auto scroll
      if (hitvidio::CurrentConfig.autoscroll == hitvidio::AUTOSCROLL_PAGE_SCROLL) {
        int playhead_x = getTimelineScreenPointFromFrame(hitvidio::ActiveSequence->playhead);
        if (playhead_x < 0 || playhead_x > (editAreas->width() - videoScrollbar->width())) {
          horizontalScrollBar->setValue(getScreenPointFromFrame(zoom, hitvidio::ActiveSequence->playhead));
          draw = false;
        }
      } else if (hitvidio::CurrentConfig.autoscroll == hitvidio::AUTOSCROLL_SMOOTH_SCROLL) {
        if (center_scroll_to_playhead(horizontalScrollBar, zoom, hitvidio::ActiveSequence->playhead)) {
          draw = false;
        }
      }
    }
  ...
  if (hitvidio::ActiveSequence != nullptr) {
    ...
    for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
      ...
      if (c != nullptr) {
        ...
        s.track = c->track();
        hitvidio::ActiveSequence->selections.append(s);
      }
    }
    repaint_timeline();
  }
...
void Timeline::select_from_playhead() {
  ...
  for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
    ClipPtr c = hitvidio::ActiveSequence->clips.at(i);
    if (c != nullptr
        && c->timeline_in() <= hitvidio::ActiveSequence->playhead
        && c->timeline_out() > hitvidio::ActiveSequence->playhead) {
      Selection s;
      s.in = c->timeline_in();
      s.out = c->timeline_out();
      s.track = c->track();
...
void Timeline::resizeEvent(QResizeEvent *) {
  ...
  if (hitvidio::ActiveSequence != nullptr) set_sb_max();
...
void Timeline::delete_in_out_internal(bool ripple) {
  if (hitvidio::ActiveSequence != nullptr && hitvidio::ActiveSequence->using_workarea) {
    QVector<Selection> areas;
    int video_tracks = 0, audio_tracks = 0;
    hitvidio::ActiveSequence->getTrackLimits(&video_tracks, &audio_tracks);
    for (int i=video_tracks;i<=audio_tracks;i++) {
      Selection s;
      s.in = hitvidio::ActiveSequence->workarea_in;
      s.out = hitvidio::ActiveSequence->workarea_out;
      s.track = i;
      areas.append(s);
    }
    ComboAction* ca = new ComboAction();
    ...
    if (ripple) ripple_clips(ca,
                             hitvidio::ActiveSequence.get(),
                             hitvidio::ActiveSequence->workarea_in,
                             hitvidio::ActiveSequence->workarea_in - hitvidio::ActiveSequence->workarea_out);
    ca->append(new SetTimelineInOutCommand(hitvidio::ActiveSequence.get(), false, 0, 0));
    hitvidio::UndoStack.push(ca);
    update_ui(true);
  }
}
...
void Timeline::toggle_enable_on_selected_clips() {
  if (hitvidio::ActiveSequence != nullptr) {
    ...
    if (!selected_clips.isEmpty()) {
      ...
      // push the action
      hitvidio::UndoStack.push(set_action);
      update_ui(false);
    }
  }
}
    ...
    if (ripple_delete) {
      ...
      for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
        ...
        if (c != nullptr && c->timeline_in() < ripple_point && c->timeline_out() > ripple_point) {
          ...
          if (!deleted) {
            for (int j=0;j<hitvidio::ActiveSequence->clips.size();j++) {
              ClipPtr cc = hitvidio::ActiveSequence->clips.at(j);
              if (cc != nullptr
                  && cc->track() == c->track()
      ...
      if (can_ripple) {
        ripple_clips(ca, hitvidio::ActiveSequence.get(), ripple_point, -ripple_length);
...
void Timeline::set_zoom_value(double v) {
  ...
  if (hitvidio::ActiveSequence != nullptr) {
    ...
    if (!horizontalScrollBar->is_resizing()) {
      center_scroll_to_playhead(horizontalScrollBar, zoom, hitvidio::ActiveSequence->playhead);
    }
  }
...
int Timeline::GetTrackHeight(int track) {
  ...
  return hitvidio::timeline::kTrackDefaultHeight;
}
...
void Timeline::ChangeTrackHeightUniformly(int diff) {
  ...
  hitvidio::ActiveSequence->getTrackLimits(&min_track, &max_track);

  // for each active track, set the track to increase/decrease based on `diff`
  for (int i=min_track;i<=max_track;i++) {
    SetTrackHeight(i, qMax(GetTrackHeight(i) + diff, hitvidio::timeline::kTrackMinHeight));
  }
...
void Timeline::IncreaseTrackHeight() {
  ChangeTrackHeightUniformly(hitvidio::timeline::kTrackHeightIncrement);
}
...
ClipPtr Timeline::split_clip(ComboAction* ca, bool transitions, int p, long frame, long post_in) {
  Clip* pre = hitvidio::ActiveSequence->clips.at(p).get();
  if (pre != nullptr) {
    ...
    if (pre->timeline_in() < frame && pre->timeline_out() > frame) {
      ...
      ClipPtr post = pre->copy(hitvidio::ActiveSequence.get());
...
bool Timeline::split_clip_and_relink(ComboAction *ca, int clip, long frame, bool relink) {
  ...
  Clip* c = hitvidio::ActiveSequence->clips.at(clip).get();
  if (c != nullptr) {
    ...
    if (post == nullptr) {
      ...
      if (relink) {
        ...
        for (int i=0;i<c->linked.size();i++) {
          ...
          if (!split_cache.contains(l)) {
            Clip* link = hitvidio::ActiveSequence->clips.at(l).get();
      ...
      ca->append(new AddClipCommand(hitvidio::ActiveSequence.get(), post_clips));
      return true;
    }
  }
  return false;
    ...
    for (int j=0;j<hitvidio::ActiveSequence->clips.size();j++) {
      ...
      if (c != nullptr && c->track() == s.track && !c->undeletable) {
        if (selection_contains_transition(s, c, kTransitionOpening)) {
          ...
          ca->append(new DeleteClipAction(hitvidio::ActiveSequence.get(), j));
        } else if (c->timeline_in() < s.in && c->timeline_out() > s.out) {
...
void Timeline::copy(bool del) {
  ...
  for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
    ClipPtr c = hitvidio::ActiveSequence->clips.at(i);
    if (c != nullptr) {
      for (int j=0;j<hitvidio::ActiveSequence->selections.size();j++) {
        const Selection& s = hitvidio::ActiveSequence->selections.at(j);
  ...
  if (del && copied) {
    delete_selection(hitvidio::ActiveSequence->selections, false);
  }
}
...
void Timeline::relink_clips_using_ids(QVector<int>& old_clips, QVector<ClipPtr>& new_clips) {
  ...
  for (int i=0;i<old_clips.size();i++) {
    ...
    ClipPtr oc = hitvidio::ActiveSequence->clips.at(old_clips.at(i));
  ...
  if (clipboard.size() > 0) {
    if (clipboard_type == CLIPBOARD_TYPE_CLIP) {
      ...
      for (int i=0;i<clipboard.size();i++) {
        ...
        ClipPtr cc = c->copy(hitvidio::ActiveSequence.get());

        // convert frame rates
        cc->set_timeline_in(rescale_frame_number(cc->timeline_in(), c->cached_frame_rate(), hitvidio::ActiveSequence->frame_rate));
        cc->set_timeline_out(rescale_frame_number(cc->timeline_out(), c->cached_frame_rate(), hitvidio::ActiveSequence->frame_rate));
        cc->set_clip_in(rescale_frame_number(cc->clip_in(), c->cached_frame_rate(), hitvidio::ActiveSequence->frame_rate));

        cc->set_timeline_in(cc->timeline_in() + hitvidio::ActiveSequence->playhead);
        cc->set_timeline_out(cc->timeline_out() + hitvidio::ActiveSequence->playhead);
      ...
      if (insert) {
        split_cache.clear();
        split_all_clips_at_point(ca, hitvidio::ActiveSequence->playhead);
        ripple_clips(ca, hitvidio::ActiveSequence.get(), paste_start, paste_end - paste_start);
      } else {
      ...
      ca->append(new AddClipCommand(hitvidio::ActiveSequence.get(), pasted_clips));

      hitvidio::UndoStack.push(ca);

      update_ui(true);

      if (hitvidio::CurrentConfig.paste_seeks) {
...
void Timeline::edit_to_point_internal(bool in, bool ripple) {
  if (hitvidio::ActiveSequence != nullptr) {
    if (hitvidio::ActiveSequence->clips.size() > 0) {
      // get track count
      ...
      for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
        ...
        if (c != nullptr) {
          ...
          if (c->timeline_in() == hitvidio::ActiveSequence->playhead)
            playhead_falls_on_in = true;
          if (c->timeline_out() == hitvidio::ActiveSequence->playhead)
      ...
      ComboAction* ca = new ComboAction();
      bool push_undo = true;
      long seek = hitvidio::ActiveSequence->playhead;

      if ((in && (playhead_falls_on_out || (playhead_falls_on_in && hitvidio::ActiveSequence->playhead == 0)))
          || (!in && (playhead_falls_on_in || (playhead_falls_on_out && hitvidio::ActiveSequence->playhead == sequence_end)))) { // one frame mode
        if (ripple) {
          // set up deletion areas based on track count
          long in_point = hitvidio::ActiveSequence->playhead;
          if (!in) {
            in_point--;
            seek--;
          }
          ...
          if (in_point >= 0) {
            ...
            if (ripple) ripple_clips(ca, hitvidio::ActiveSequence.get(), in_point, -1);
          } else {
            push_undo = false;
          }
        } else {
        ...
        Selection s;
        if (in) seek = prev_cut;
        s.in = in ? prev_cut : hitvidio::ActiveSequence->playhead;
        s.out = in ? hitvidio::ActiveSequence->playhead : next_cut;
        ...
        if (s.in == s.out) {
          ...
          // trim and move clips around the in point
          delete_areas_and_relink(ca, areas, true);
          if (ripple) ripple_clips(ca, hitvidio::ActiveSequence.get(), s.in, s.in - s.out);
        }
      }
      ...
      if (push_undo) {
        ...
        if (seek != hitvidio::ActiveSequence->playhead && ripple) {
...
bool Timeline::split_selection(ComboAction* ca) {
  ...
  for (int j=0;j<hitvidio::ActiveSequence->clips.size();j++) {
    ClipPtr clip = hitvidio::ActiveSequence->clips.at(j);
    if (clip != nullptr) {
      for (int i=0;i<hitvidio::ActiveSequence->selections.size();i++) {
        const Selection& s = hitvidio::ActiveSequence->selections.at(i);
  ...
  if (split) {
    ...
    ca->append(new AddClipCommand(hitvidio::ActiveSequence.get(), post_splits));
    ca->append(new AddClipCommand(hitvidio::ActiveSequence.get(), secondary_post_splits));
...
bool Timeline::split_all_clips_at_point(ComboAction* ca, long point) {
  bool split = false;
  for (int j=0;j<hitvidio::ActiveSequence->clips.size();j++) {
    ClipPtr c = hitvidio::ActiveSequence->clips.at(j);
    if (c != nullptr) {
      // always relinks
...
void Timeline::split_at_playhead() {
  ...
  if (hitvidio::ActiveSequence->selections.size() > 0) {
    // see if whole clips are selected
    QVector<int> pre_clips;
    QVector<ClipPtr> post_clips;
    for (int j=0;j<hitvidio::ActiveSequence->clips.size();j++) {
      Clip* clip = hitvidio::ActiveSequence->clips.at(j).get();
      if (clip != nullptr && clip->IsSelected()) {
        ClipPtr s = split_clip(ca, true, j, hitvidio::ActiveSequence->playhead);
        if (s != nullptr) {
          pre_clips.append(j);
          post_clips.append(s);
          split_selected = true;
  ...
  if (!split_selected) {
    split_selected = split_all_clips_at_point(ca, hitvidio::ActiveSequence->playhead);
  }
...
void Timeline::ripple_delete() {
  if (hitvidio::ActiveSequence != nullptr) {
    if (hitvidio::ActiveSequence->selections.size() > 0) {
      panel_timeline->delete_selection(hitvidio::ActiveSequence->selections, true);
    } else if (hitvidio::CurrentConfig.hover_focus && get_focused_panel() == panel_timeline) {
...
void Timeline::deselect_area(long in, long out, int track) {
  int len = hitvidio::ActiveSequence->selections.size();
  for (int i=0;i<len;i++) {
    Selection& s = hitvidio::ActiveSequence->selections[i];
    if (s.track == track) {
      if (s.in >= in && s.out <= out) {
        // whole selection is in deselect area
        hitvidio::ActiveSequence->selections.removeAt(i);
        i--;
        len--;
        ...
        hitvidio::ActiveSequence->selections.append(new_sel);
...
bool Timeline::snap_to_timeline(long* l, bool use_playhead, bool use_markers, bool use_workarea) {
  ...
  if (snapping) {
    if (use_playhead && !panel_sequence_viewer->playing) {
      // snap to playhead
      if (snap_to_point(hitvidio::ActiveSequence->playhead, l)) return true;
    }

    // snap to marker
    if (use_markers) {
      for (int i=0;i<hitvidio::ActiveSequence->markers.size();i++) {
        if (snap_to_point(hitvidio::ActiveSequence->markers.at(i).frame, l)) return true;
      }
    }

    // snap to in/out
    if (use_workarea && hitvidio::ActiveSequence->using_workarea) {
      if (snap_to_point(hitvidio::ActiveSequence->workarea_in, l)) return true;
      if (snap_to_point(hitvidio::ActiveSequence->workarea_out, l)) return true;
    }
...
void Timeline::set_marker() {
  ...
  for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
    ...
    if (c != nullptr
      ...
      if (hitvidio::ActiveSequence->playhead >= c->timeline_in()
          && hitvidio::ActiveSequence->playhead <= c->timeline_out()) {
  ...
  set_marker_internal(hitvidio::ActiveSequence.get(), clips_selected);
...
void Timeline::toggle_links() {
  ...
  command->s = hitvidio::ActiveSequence.get();
  for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
    Clip* c = hitvidio::ActiveSequence->clips.at(i).get();
    if (c != nullptr && c->IsSelected()) {
  ...
  if (command->clips.size() > 0) {
    hitvidio::UndoStack.push(command);
    repaint_timeline();
  } else {
    delete command;
  }
...
void Timeline::record_btn_click() {
  if (hitvidio::ActiveProjectFilename.isEmpty()) {
    ...
    hitvidio::MainWindow->statusBar()->showMessage(
...
void Timeline::set_sb_max() {
  headers->set_scrollbar_max(horizontalScrollBar, hitvidio::ActiveSequence->getEndFrame(), editAreas->width() - getScreenPointFromFrame(zoom, 200));
}
...
void Timeline::UpdateTitle() {
  ...
  if (hitvidio::ActiveSequence == nullptr) {
    ...
    setWindowTitle(title + hitvidio::ActiveSequence->name);
    update_ui(false);
  }
}
...
void Timeline::setup_ui() {
  ...
  toolArrowButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/arrow.svg")));
  ...
  toolEditButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/beam.svg")));
  ...
  toolRippleButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/ripple.svg")));
  ...
  toolRazorButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/razor.svg")));
  ...
  toolSlipButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/slip.svg")));
  ...
  toolSlideButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/slide.svg")));
  ...
  toolHandButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/hand.svg")));
  ...
  toolTransitionButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/transition-tool.svg")));
  ...
  snappingButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/magnet.svg")));
  ...
  zoomInButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/zoomin.svg")));
  ...
  zoomOutButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/zoomout.svg")));
  connect(zoomOutButton, SIGNAL(clicked(bool)), this, SLOT(zoom_out()));
  tool_buttons_layout->addWidget(zoomOutButton);

  recordButton = new QPushButton();
  recordButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/record.svg")));
  connect(recordButton, SIGNAL(clicked(bool)), this, SLOT(record_btn_click()));
  tool_buttons_layout->addWidget(recordButton);

  addButton = new QPushButton();
  addButton->setIcon(hitvidio::icon::CreateIconFromSVG(QStringLiteral(":/icons/add-button.svg")));
...
void Timeline::set_tool() {
  ...
  case TIMELINE_TOOL_RAZOR:
    timeline_area->setCursor(hitvidio::cursor::Razor);
    break;
...
void hitvidio::timeline::MultiplyTrackSizesByDPI()
{
