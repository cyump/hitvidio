  ...
  QThread(nullptr)
{
  ...
  hitvidio::media_icon_service->SetMediaIcon(media_, ICON_TYPE_LOADING);
...
void PreviewGenerator::finalize_media() {
  if (!cancelled_) {
    ...
    if (footage_->video_tracks.isEmpty() && footage_->audio_tracks.isEmpty()) {
      ...
      // VIDEO
      hitvidio::media_icon_service->SetMediaIcon(media_, ICON_TYPE_VIDEO);
    } else if (!footage_->audio_tracks.isEmpty()) {
      // AUDIO
      hitvidio::media_icon_service->SetMediaIcon(media_, ICON_TYPE_AUDIO);
    } else {
      // IMAGE
      hitvidio::media_icon_service->SetMediaIcon(media_, ICON_TYPE_IMAGE);
    }
    ...
    if (hitvidio::ActiveSequence != nullptr) {
      hitvidio::ActiveSequence->RefreshClips(media_);
    }
  }
}
...
void PreviewGenerator::invalidate_media(const QString &error_msg)
{
  ...
  hitvidio::media_icon_service->SetMediaIcon(media_, ICON_TYPE_ERROR);
...
void PreviewGenerator::generate_waveform() {
  ...
  for (unsigned int i=0;i<fmt_ctx_->nb_streams;i++) {
    ...
    if ((fmt_ctx_->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO && hitvidio::CurrentConfig.thumbnail_resolution > 0)
        || (fmt_ctx_->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO && hitvidio::CurrentConfig.waveform_resolution > 0)) {
      ...
      if (!end_of_file) {
        ...
        if (s != nullptr) {
          if (fmt_ctx_->streams[packet->stream_index]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            if (!s->preview_done) {
              int dstH = hitvidio::CurrentConfig.thumbnail_resolution;
            ...
            int interval = qFloor((temp_frame->sample_rate/hitvidio::CurrentConfig.waveform_resolution)/4)*4;
