...
void Media::set_sequence(SequencePtr s) {
  set_icon(hitvidio::icon::MediaSequence);
...
void Media::set_folder() {
  ...
  set_icon(hitvidio::icon::MediaFolder);
...
bool Media::setData(int col, const QVariant &value) {
  if (col == 0) {
    ...
    if (!n.isEmpty() && get_name() != n) {
      hitvidio::UndoStack.push(new MediaRename(this, value.toString()));
      return true;
    }
  }
  return false;
...
QString Media::GetStringDuration() {
  if (get_type() == MEDIA_TYPE_SEQUENCE) {
    Sequence* s = to_sequence().get();
    return frame_to_timecode(s->getEndFrame(), hitvidio::CurrentConfig.timecode_view, s->frame_rate);
  }
  if (get_type() == MEDIA_TYPE_FOOTAGE) {
    ...
    if (len > 0) return frame_to_timecode(len, hitvidio::CurrentConfig.timecode_view, r);
  }
  return QString();
}
