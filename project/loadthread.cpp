...
bool LoadThread::load_worker(QFile& f, QXmlStreamReader& stream, int type) {
  ...
  int proj_version = hitvidio::kSaveVersion;
  ...
  while (!stream.atEnd() && !cancelled_) {
    ...
    if (stream.name() == root_search) {
      if (type == LOAD_TYPE_VERSION) {
        ...
        if (proj_version < hitvidio::kMinimumSaveVersion || proj_version > hitvidio::kSaveVersion) {
          show_message(
                  tr("Version Mismatch"),
                  tr("This project was saved in a different version of Hitvidio and may not be fully compatible with this version. Would you like to attempt loading it anyway?"),
        ...
        while (!cancelled_ && !stream.atEnd() && !(stream.name() == root_search && stream.isEndElement())) {
          ...
          if (stream.name() == child_search && stream.isStartElement()) {
            ...
            case MEDIA_TYPE_FOOTAGE:
            {
              ...
              hitvidio::project_model.appendChild(find_loaded_folder_by_id(folder), item);
...
void LoadThread::OrganizeFolders(int folder) {
  for (int i=0;i<loaded_folders.size();i++) {
    ...
    if (parent_id == folder) {
      hitvidio::project_model.appendChild(find_loaded_folder_by_id(parent_id), item);
...
void LoadThread::success_func() {
  if (autorecovery_) {
    ...
    hitvidio::Global->update_project_filename(orig_filename);
  } else {
    panel_project->add_recent_project(filename_);
  }

  hitvidio::Global->set_modified(autorecovery_);
  if (open_seq != nullptr) {
    hitvidio::Global->set_sequence(open_seq);
  }
}