    ...
    if (all_footage) {
      ...
      if (cached_selected_footage.size() == 1
        ...
        QAction* action = proxies->addAction(tr("Generating proxy: %1% complete").arg(
                                               hitvidio::proxy_generator.get_proxy_progress(cached_selected_footage.at(0))
                                               )
                                             );
...
void SourcesCommon::mouseDoubleClickEvent(const QModelIndexList& selected_items) {
  ...
  if (selected_items.size() == 0) {
    ...
    if (media->get_type() == MEDIA_TYPE_SEQUENCE) {
      hitvidio::UndoStack.push(new ChangeSequenceAction(media->to_sequence()));
    } else {
  ...
  if (mimeData->hasUrls()) {
    ...
    if (!urls.isEmpty()) {
      ...
      if (urls.size() == 1
          ...
          && hitvidio::CurrentConfig.drop_on_media_to_replace
...
void SourcesCommon::open_create_proxy_dialog() {
  ...
  ProxyDialog pd(hitvidio::MainWindow, cached_selected_footage);
  pd.exec();
}
  ...
  for (int i=0;i<cached_selected_footage.size();i++) {
    ...
    if (f->proxy && !f->proxy_path.isEmpty()) {
      if (QFileInfo::exists(f->proxy_path)) {
        if (QMessageBox::question(hitvidio::MainWindow,
                                  tr("Delete proxy"),
