...
void ChangeSequenceAction::doUndo() {
  hitvidio::Global->set_sequence(old_sequence);
}

void ChangeSequenceAction::doRedo() {
  old_sequence = hitvidio::ActiveSequence;
  hitvidio::Global->set_sequence(new_sequence);
}
...
void AddMediaCommand::doUndo() {
  hitvidio::project_model.removeChild(parent, item.get());
  done_ = false;
}
...
void MediaMove::doUndo() {
  for (int i=0;i<items.size();i++) {
    hitvidio::project_model.moveChild(items.at(i), froms.at(i));
  }
}

void MediaMove::doRedo() {
  if (to == nullptr) to = hitvidio::project_model.get_root();
  froms.resize(items.size());
  for (int i=0;i<items.size();i++) {
    Media* parent = items.at(i)->parentItem();
    froms[i] = parent;
    hitvidio::project_model.moveChild(items.at(i), to);
  }
}
...
void EditSequenceCommand::update() {
  ...
  if (hitvidio::ActiveSequence == seq) {
    hitvidio::Global->set_sequence(seq);
  }
}
...
HitvidioAction::HitvidioAction(bool iset_window_modified) {
  set_window_modified = iset_window_modified;
}

HitvidioAction::~HitvidioAction() {}

void HitvidioAction::undo() {
  doUndo();

  if (set_window_modified) {
    hitvidio::Global->set_modified(old_window_modified);
  }
}

void HitvidioAction::redo() {
  doRedo();

  if (set_window_modified) {

    // store current modified state
    old_window_modified = hitvidio::Global->is_modified();

    // set modified to true
    hitvidio::Global->set_modified(true);
