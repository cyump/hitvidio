...
void process_effect(Clip* c,
  ...
  if (e->IsEnabled()) {
    ...
    bool can_process_shaders = ((e->Flags() & Effect::ShaderFlag) && hitvidio::CurrentRuntimeConfig.shaders_are_enabled);
...
GLuint hitvidio::rendering::compose_sequence(ComposeSequenceParams &params) {
      ...
      if (c->track() < 0) {
        ...
        if (playhead >= c->timeline_in(true)
          ...
          if (textureID > 0) {
            ...
            if (!hitvidio::CurrentRuntimeConfig.disable_blending) {
              if (params.nests.size() > 0) {
...
void hitvidio::rendering::compose_audio(Viewer* viewer, Sequence* seq, int playback_speed, bool wait_for_mutexes) {
