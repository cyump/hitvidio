...
void Cacher::CacheAudioWorker() {
  ...
  if (temp_reverse) {
    long seq_end = hitvidio::ActiveSequence->getEndFrame();
...
void Cacher::CacheVideoWorker() {
  ...
  if (clip->media_stream()->infinite_length) {
    ...
    // frames now
    if (reversed) {
      previous_queue_type = hitvidio::CurrentConfig.upcoming_queue_type;
      previous_queue_size = hitvidio::CurrentConfig.upcoming_queue_size;
      upcoming_queue_type = hitvidio::CurrentConfig.previous_queue_type;
      upcoming_queue_size = hitvidio::CurrentConfig.previous_queue_size;
    } else {
      previous_queue_type = hitvidio::CurrentConfig.previous_queue_type;
      previous_queue_size = hitvidio::CurrentConfig.previous_queue_size;
      upcoming_queue_type = hitvidio::CurrentConfig.upcoming_queue_type;
      upcoming_queue_size = hitvidio::CurrentConfig.upcoming_queue_size;
    }

    // Determine "previous" queue statistics
    if (previous_queue_type == hitvidio::FRAME_QUEUE_TYPE_FRAMES) {
    ...
    if (upcoming_queue_type == hitvidio::FRAME_QUEUE_TYPE_FRAMES) {
    ...
    if ((upcoming_queue_type == hitvidio::FRAME_QUEUE_TYPE_FRAMES && frames_greater_than_target >= maximum_ts)
        || (upcoming_queue_type == hitvidio::FRAME_QUEUE_TYPE_SECONDS && latest_pts > maximum_ts)) {
      start_loop = false;
    }
    ...
    if (start_loop) {
      ...
      do {
        ...
        if (retrieve_code < 0 && retrieve_code != AVERROR_EOF) {
          ...
          if (previous_queue_type == hitvidio::FRAME_QUEUE_TYPE_SECONDS
            ...
            // check if the queue is full according to hitvidio::CurrentConfig
            if (upcoming_queue_type == hitvidio::FRAME_QUEUE_TYPE_FRAMES) {
            ...
            } else if (decoded_frame->pts > maximum_ts) { // for `upcoming_queue_type == hitvidio::FRAME_QUEUE_TYPE_SECONDS`
              break;
            }
          ...
          if (retrieve_code == AVERROR_EOF && retrieved_frame == nullptr && !queue_.isEmpty()) {
            ...
            // some formats EOF before the end of the duration that Hitvidio calculates. In this event, we simply
...
void Cacher::OpenWorker() {
  ...
  if (clip->media() == nullptr) {
    ...
    if (errCode != 0) {
      ...
      hitvidio::MainWindow->statusBar()->showMessage(tr("Could not open %1 - %2").arg(filename, err));
      return;
    }
