...
QAudioDeviceInfo get_audio_device(QAudio::Mode mode) {
  ...
  QString preferred_device = (mode == QAudio::AudioOutput) ? hitvidio::CurrentConfig.preferred_audio_output : hitvidio::CurrentConfig.preferred_audio_input;
...
void init_audio() {
  ...
  audio_format.setSampleRate(hitvidio::CurrentConfig.audio_rate);
...
bool start_recording() {
  ...
  QString audio_path = QCoreApplication::translate("Audio", "%1 Audio").arg(hitvidio::ActiveProjectFilename);
  ...
  if (!output_recording.open(QFile::WriteOnly)) {
    qCritical() << "Failed to open output file. Does Hitvidio have permission to write to this directory?";
    return false;
  }

  QAudioFormat audio_format = audio_output->format();
  if (hitvidio::CurrentConfig.recording_mode != audio_format.channelCount()) {
    audio_format.setChannelCount(hitvidio::CurrentConfig.recording_mode);
  }
