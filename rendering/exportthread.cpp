...
bool ExportThread::SetupVideo() {
  ...
  video_frame->width = hitvidio::ActiveSequence->width;
  video_frame->height = hitvidio::ActiveSequence->height;
  ...
  sws_ctx = sws_getContext(
        hitvidio::ActiveSequence->width,
        hitvidio::ActiveSequence->height,
        AV_PIX_FMT_RGBA,
        params_.video_width,
...
bool ExportThread::SetupAudio() {
  ...
  swr_ctx = swr_alloc_set_opts(
        ...
        hitvidio::ActiveSequence->audio_layout,
        AV_SAMPLE_FMT_S16,
        acodec_ctx->sample_rate,
        0,
        nullptr
...
void ExportThread::Export()
{
  ...
  while (hitvidio::ActiveSequence->playhead <= params_.end_frame && !interrupt_) {
    ...
    if (params_.audio_enabled) {
      ...
      hitvidio::rendering::compose_audio(nullptr, hitvidio::ActiveSequence.get(), 1, true);
    }
    ...
    if (params_.video_enabled) {
      do {
        ...
        renderer->start_render(nullptr, hitvidio::ActiveSequence.get(), 1, nullptr, video_frame->data[0], video_frame->linesize[0]/4);
    ...
    double timecode_secs = double(hitvidio::ActiveSequence->playhead - params_.start_frame) / hitvidio::ActiveSequence->frame_rate;
    ...
    remaining_frames = (params_.end_frame - hitvidio::ActiveSequence->playhead);
    avg_time = (total_time/frame_count);
    eta = (remaining_frames*avg_time);

    // Emit a signal for the percent of the sequence that's been encoded so far
    emit ProgressChanged(qRound((double(hitvidio::ActiveSequence->playhead - params_.start_frame) / double(params_.end_frame - params_.start_frame)) * 100.0), eta);

    // Increment sequence playhead
    hitvidio::ActiveSequence->playhead++;
  ...
  hitvidio::Global->set_rendering_state(false);
  close_active_clips(hitvidio::ActiveSequence.get());
