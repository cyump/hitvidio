...
EffectPtr Effect::Create(Clip* c, const EffectMeta* em) {
  if (em->internal >= 0 && em->internal < EFFECT_INTERNAL_COUNT) {
    ...
    QMessageBox::critical(hitvidio::MainWindow,
                          ...
                          QCoreApplication::translate("Effect", "No candidate for effect '%1'. This effect may be corrupt. Try reinstalling it or Hitvidio.").arg(em->name));
  }
  return nullptr;
}
...
void Effect::delete_self() {
  hitvidio::UndoStack.push(new EffectDeleteCommand(this));
  update_ui(true);
}
...
void Effect::save_to_file() {
  ...
  QString file = QFileDialog::getSaveFileName(hitvidio::MainWindow,
...
void Effect::load_from_file() {
  ...
  QString file = QFileDialog::getOpenFileName(hitvidio::MainWindow,
  ...
  if (!file.isEmpty()) {
    ...
    if (file_handle.open(QFile::ReadOnly)) {
      ...
      hitvidio::UndoStack.push(new SetEffectData(this, file_handle.readAll()));
...
void Effect::open() {
  ...
  if (hitvidio::CurrentRuntimeConfig.shaders_are_enabled && (Flags() & ShaderFlag)) {
...
void Effect::startEffect() {
  ...
  if (hitvidio::CurrentRuntimeConfig.shaders_are_enabled
