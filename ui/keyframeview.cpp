...
void KeyframeView::paintEvent(QPaintEvent*) {
  ...
  if (!open_effects_.isEmpty()) {
    ...
    int playhead_x = getScreenPointFromFrame(panel_effect_controls->zoom, hitvidio::ActiveSequence->playhead-visible_in) - x_scroll;
    ...
    if (select_rect) {
      ...
      if (panel_timeline->snapping) {
        for (int i=0;i<selected_keyframes.size();i++) {
          ...
          if (panel_timeline->snap_to_point(hitvidio::ActiveSequence->playhead, &key_eval)) {
