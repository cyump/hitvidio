...
MenuHelper hitvidio::MenuHelper;

void MenuHelper::InitializeSharedMenus()
{
  new_project_ = create_menu_action(nullptr, "newproj", hitvidio::Global.get(), SLOT(new_project()), QKeySequence("Ctrl+N"));
  ...
  set_in_point_ = create_menu_action(nullptr, "setinpoint", &hitvidio::FocusFilter, SLOT(set_in_point()), QKeySequence("I"));
  set_in_point_->setParent(this);

  set_out_point_ = create_menu_action(nullptr, "setoutpoint", &hitvidio::FocusFilter, SLOT(set_out_point()), QKeySequence("O"));
  set_out_point_->setParent(this);

  reset_in_point_ = create_menu_action(nullptr, "resetin", &hitvidio::FocusFilter, SLOT(clear_in()));
  reset_in_point_->setParent(this);

  reset_out_point_ = create_menu_action(nullptr, "resetout", &hitvidio::FocusFilter, SLOT(clear_out()));
  reset_out_point_->setParent(this);

  clear_inout_point = create_menu_action(nullptr, "clearinout", &hitvidio::FocusFilter, SLOT(clear_inout()), QKeySequence("G"));
  ...
  cut_ = create_menu_action(nullptr, "cut", &hitvidio::FocusFilter, SLOT(cut()), QKeySequence("Ctrl+X"));
  cut_->setParent(this);

  copy_ = create_menu_action(nullptr, "copy", &hitvidio::FocusFilter, SLOT(copy()), QKeySequence("Ctrl+C"));
  copy_->setParent(this);

  paste_ = create_menu_action(nullptr, "paste", hitvidio::Global.get(), SLOT(paste()), QKeySequence("Ctrl+V"));
  paste_->setParent(this);

  paste_insert_ = create_menu_action(nullptr, "pasteinsert", hitvidio::Global.get(), SLOT(paste_insert()), QKeySequence("Ctrl+Shift+V"));
  paste_insert_->setParent(this);

  duplicate_ = create_menu_action(nullptr, "duplicate", &hitvidio::FocusFilter, SLOT(duplicate()), QKeySequence("Ctrl+D"));
  duplicate_->setParent(this);

  delete_ = create_menu_action(nullptr, "delete", &hitvidio::FocusFilter, SLOT(delete_function()), QKeySequence("Del"));
...
void MenuHelper::set_titlesafe_from_menu() {
  ...
  if (qIsNaN(tsa)) {
    ...
    hitvidio::CurrentConfig.show_title_safe_area = false;

  } else {

    // using title safe area
    hitvidio::CurrentConfig.show_title_safe_area = true;

    // are we using the default area aspect ratio, or a specific one
    if (qIsNull(tsa)) {

      // default title safe area
      hitvidio::CurrentConfig.use_custom_title_safe_ratio = false;

    } else {

      // using a specific aspect ratio
      hitvidio::CurrentConfig.use_custom_title_safe_ratio = true;
      ...
      if (tsa < 0.0) {
        ...
        do {
          if (invalid) {
            QMessageBox::critical(hitvidio::MainWindow, tr("Invalid aspect ratio"), tr("The aspect ratio '%1' is invalid. Please try again.").arg(input));
          }

          input = QInputDialog::getText(hitvidio::MainWindow, tr("Enter custom aspect ratio"), tr("Enter the aspect ratio to use for the title/action safe area (e.g. 16:9):"));
          invalid = !arTest.exactMatch(input) && !input.isEmpty();
        } while (invalid);

        if (!input.isEmpty()) {
          QStringList inputList = input.split(':');
          hitvidio::CurrentConfig.custom_title_safe_ratio = inputList.at(0).toDouble()/inputList.at(1).toDouble();
        }

      } else {

        // specified tsa is a specific custom aspect ratio
        hitvidio::CurrentConfig.custom_title_safe_ratio = tsa;
      }
...
void MenuHelper::set_autoscroll() {
  QAction* action = static_cast<QAction*>(sender());
  hitvidio::CurrentConfig.autoscroll = action->data().toInt();
}
...
void MenuHelper::set_timecode_view() {
  QAction* action = static_cast<QAction*>(sender());
  hitvidio::CurrentConfig.timecode_view = action->data().toInt();
  update_ui(false);
}

void MenuHelper::open_recent_from_menu() {
  int index = static_cast<QAction*>(sender())->data().toInt();
  hitvidio::Global.get()->open_recent(index);
}
