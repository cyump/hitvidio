...
void TimelineWidget::show_context_menu(const QPoint& pos) {
  if (hitvidio::ActiveSequence != nullptr) {
    ...
    connect(undoAction, SIGNAL(triggered(bool)), hitvidio::Global.get(), SLOT(undo()));
    connect(redoAction, SIGNAL(triggered(bool)), hitvidio::Global.get(), SLOT(redo()));
    undoAction->setEnabled(hitvidio::UndoStack.canUndo());
    redoAction->setEnabled(hitvidio::UndoStack.canRedo());
    menu.addSeparator();

    // collect all the selected clips
    QVector<Clip*> selected_clips = hitvidio::ActiveSequence->SelectedClips();

    hitvidio::MenuHelper.make_edit_functions_menu(&menu, !selected_clips.isEmpty());
    ...
    if (!selected_clips.isEmpty()) {
      ...
      menu.addAction(tr("&Speed/Duration"), hitvidio::Global.get(), SLOT(open_speed_dialog()));

      if (audio_clips_are_selected) {
        menu.addAction(tr("Auto-Cut Silence"), hitvidio::Global.get(), SLOT(open_autocut_silence_dialog()));
      }
      ...
      hitvidio::MenuHelper.make_clip_functions_menu(&menu);
...
void TimelineWidget::toggle_autoscale() {
  ...
  if (!selected_clips.isEmpty()) {
    ...
    hitvidio::UndoStack.push(action);
  }
}

void TimelineWidget::tooltip_timer_timeout() {
  if (hitvidio::ActiveSequence != nullptr) {
    if (tooltip_clip < hitvidio::ActiveSequence->clips.size()) {
      ClipPtr c = hitvidio::ActiveSequence->clips.at(tooltip_clip);
      if (c != nullptr) {
        QToolTip::showText(QCursor::pos(),
                           tr("%1\nStart: %2\nEnd: %3\nDuration: %4").arg(
                             c->name(),
                             frame_to_timecode(c->timeline_in(), hitvidio::CurrentConfig.timecode_view, hitvidio::ActiveSequence->frame_rate),
                             frame_to_timecode(c->timeline_out(), hitvidio::CurrentConfig.timecode_view, hitvidio::ActiveSequence->frame_rate),
                             frame_to_timecode(c->length(), hitvidio::CurrentConfig.timecode_view, hitvidio::ActiveSequence->frame_rate)
                             ));
      }
    }
  }
...
void TimelineWidget::open_sequence_properties() {
  ...
  for (int i=0;i<hitvidio::project_model.childCount();i++) {
    all_top_level_items.append(hitvidio::project_model.child(i));
  }
  ...
  for (int i=0;i<sequence_items.size();i++) {
    if (sequence_items.at(i)->to_sequence() == hitvidio::ActiveSequence) {
...
void TimelineWidget::dragEnterEvent(QDragEnterEvent *event) {
  ...
  if (event->source() == panel_footage_viewer) {
    if (panel_footage_viewer->seq != hitvidio::ActiveSequence) { // don't allow nesting the same sequence

      media_list.append(hitvidio::timeline::MediaImportData(panel_footage_viewer->media,
                         static_cast<hitvidio::timeline::MediaImportType>(event->mimeData()->text().toInt())));
      import_init = true;

    }
  }

  if (hitvidio::CurrentConfig.enable_drag_files_to_timeline && event->mimeData()->hasUrls()) {
  ...
  if (import_init) {
    ...
    long entry_point;
    Sequence* seq = hitvidio::ActiveSequence.get();
...
void TimelineWidget::wheelEvent(QWheelEvent *event) {
  ...
  bool zooming = (hitvidio::CurrentConfig.scroll_zooms != ctrl);

  // Allow shift for axis swap, but don't swap on zoom... Unless
  // we need to override Qt's axis swap via Alt
  bool swap_hv = ((shift != hitvidio::CurrentConfig.invert_timeline_scroll_axes) &
...
void insert_clips(ComboAction* ca) {
  ...
  ripple_clips(ca, hitvidio::ActiveSequence.get(), earliest_new_point, ripple_length, ignore_clips);
  ...
  if (ripple_old_point) {
    ...
    ripple_clips(ca, hitvidio::ActiveSequence.get(), latest_old_point, second_ripple_length, ignore_clips);
...
void TimelineWidget::dropEvent(QDropEvent* event) {
  if (panel_timeline->importing && panel_timeline->ghosts.size() > 0) {
    ...
    Sequence* s = hitvidio::ActiveSequence.get();
...
void TimelineWidget::mouseDoubleClickEvent(QMouseEvent *event) {
  if (hitvidio::ActiveSequence != nullptr) {
    if (panel_timeline->tool == TIMELINE_TOOL_EDIT) {
      ...
      if (clip_index >= 0) {
        ClipPtr clip = hitvidio::ActiveSequence->clips.at(clip_index);
        if (!(event->modifiers() & Qt::ShiftModifier)) hitvidio::ActiveSequence->selections.clear();
        Selection s;
      ...
      int clip_index = getClipIndexFromCoords(panel_timeline->cursor_frame, panel_timeline->cursor_track);
      if (clip_index >= 0) {
        ClipPtr c = hitvidio::ActiveSequence->clips.at(clip_index);
        if (c->media() != nullptr && c->media()->get_type() == MEDIA_TYPE_SEQUENCE) {
          hitvidio::Global->set_sequence(c->media()->to_sequence());
        }
      }
    }
  }
...
void TimelineWidget::mousePressEvent(QMouseEvent *event) {
  if (hitvidio::ActiveSequence != nullptr) {
    ...
    // any prior to that)
    if (shift) {
      panel_timeline->selection_offset = hitvidio::ActiveSequence->selections.size();
    } else {
    ...
    if (panel_timeline->creating) {
      ...
      case TIMELINE_TOOL_MENU:
      {
        if (track_resizing && effective_tool != TIMELINE_TOOL_MENU) {
          ...
          if (hovered_clip >= 0) {
            Clip* clip = hitvidio::ActiveSequence->clips.at(hovered_clip).get();
            ...
            if (clip->IsSelected()) {
              ...
              if (shift) {
                ...
                if (!alt) {
                  for (int i=0;i<clip->linked.size();i++) {
                    ClipPtr link = hitvidio::ActiveSequence->clips.at(clip->linked.at(i));
              ...
              if (hitvidio::CurrentConfig.select_also_seeks) {
              ...
              if (!alt && panel_timeline->transition_select == kTransitionNone) {
                ...
                for (int i=0;i<clip->linked.size();i++) {
                  ...
                  Clip* link = hitvidio::ActiveSequence->clips.at(clip->linked.at(i)).get();
                  ...
                  if (!link->IsSelected()) {
                    ...
                    ss.track = link->track();
                    hitvidio::ActiveSequence->selections.append(ss);
                  }
      ...
      case TIMELINE_TOOL_EDIT:
        ...
        if (hitvidio::CurrentConfig.edit_tool_also_seeks) {
...
void TimelineWidget::mouseReleaseEvent(QMouseEvent *event) {
  ...
  if (hitvidio::ActiveSequence != nullptr) {
    ...
    if (event->button() == Qt::LeftButton) {
      ...
      if (panel_timeline->creating) {
        if (panel_timeline->ghosts.size() > 0) {
          ...
          if (panel_timeline->creating_object == ADD_OBJ_AUDIO) {
            hitvidio::MainWindow->statusBar()->clearMessage();
            panel_sequence_viewer->cue_recording(qMin(g.in, g.out), qMax(g.in, g.out), g.track);
            panel_timeline->creating = false;
          } else if (g.in != g.out) {
            ClipPtr c = std::make_shared<Clip>(hitvidio::ActiveSequence.get());
            ...
            QVector<ClipPtr> add;
            add.append(c);
            ca->append(new AddClipCommand(hitvidio::ActiveSequence.get(), add));

            if (c->track() < 0 && hitvidio::CurrentConfig.add_default_effects_to_clips) {
        ...
        if (process_moving) {
          ...
          if (panel_timeline->tool == TIMELINE_TOOL_RIPPLE) {
            ...
            if (panel_timeline->trim_type == TRIM_IN) {
              ...
              for (int i=0;i<hitvidio::ActiveSequence->selections.size();i++) {
                hitvidio::ActiveSequence->selections[i].in += ripple_length;
                hitvidio::ActiveSequence->selections[i].out += ripple_length;
              }
            } else {
            ...
            ripple_clips(ca, hitvidio::ActiveSequence.get(), ripple_point, ripple_length, ignore_clips);
          }
          ...
          if (panel_timeline->tool == TIMELINE_TOOL_POINTER
            ...
            for (int i=0;i<panel_timeline->ghosts.size();i++) {
              ...
              if (g.old_in != g.in || g.old_out != g.out || g.track != g.old_track || g.clip_in != g.old_clip_in) {
                ...
                // create copy of clip
                ClipPtr c = hitvidio::ActiveSequence->clips.at(g.clip)->copy(hitvidio::ActiveSequence.get());
            ...
            if (new_clips.size() > 0) {
              ...
              // add them
              ca->append(new AddClipCommand(hitvidio::ActiveSequence.get(), new_clips));
            ...
            if (panel_timeline->tool == TIMELINE_TOOL_POINTER && ctrl) {
              ...
              for (int i=0;i<panel_timeline->ghosts.size();i++) {
                ...
                hitvidio::ActiveSequence->clips.at(g.clip)->undeletable = true;
              ...
              for (int i=0;i<panel_timeline->ghosts.size();i++) {
                const Ghost& g = panel_timeline->ghosts.at(i);
                hitvidio::ActiveSequence->clips.at(g.clip)->undeletable = false;
            ...
            for (int i=0;i<panel_timeline->ghosts.size();i++) {
              ...
              Clip* c = hitvidio::ActiveSequence->clips.at(g.clip).get();
            ...
            for (int i=0;i<panel_timeline->ghosts.size();i++) {
              ...
              if (g.transition == nullptr) {
                ClipPtr c = hitvidio::ActiveSequence->clips.at(g.clip);
                    ...
                    if (transition->secondary_clip != nullptr) {
                      ...
                      if ((t == kTransitionOpening && g.in != g.old_in)
                        ...
                        for (int j=0;j<panel_timeline->ghosts.size();j++) {
                          ...
                          if (hitvidio::ActiveSequence->clips.at(other_clip_ghost.clip).get() == search_clip) {
        ...
        if (g.in != g.out) {
          ...
          Clip* open = (panel_timeline->transition_tool_open_clip > -1)
              ? hitvidio::ActiveSequence->clips.at(panel_timeline->transition_tool_open_clip).get()
              : nullptr;

          Clip* close = (panel_timeline->transition_tool_close_clip > -1)
              ? hitvidio::ActiveSequence->clips.at(panel_timeline->transition_tool_close_clip).get()
              : nullptr;
      ...
      panel_timeline->clean_up_selections(hitvidio::ActiveSequence->selections);

      if (selection_command != nullptr) {
        selection_command->new_data = hitvidio::ActiveSequence->selections;
...
void TimelineWidget::update_ghosts(const QPoint& mouse_pos, bool lock_frame) {
  ...
  for (int i=0;i<panel_timeline->ghosts.size();i++) {
    ...
    Clip* c = nullptr;
    if (g.clip != -1) {
      c = hitvidio::ActiveSequence->clips.at(g.clip).get();
    }
    ...
    if (panel_timeline->creating) {
      ...
      if (panel_timeline->transition_tool_open_clip == -1
        ...
        Clip* otc = hitvidio::ActiveSequence->clips.at(panel_timeline->transition_tool_open_clip).get();

        // close transition clip
        Clip* ctc = hitvidio::ActiveSequence->clips.at(panel_timeline->transition_tool_close_clip).get();
  ...
  for (int i=0;i<panel_timeline->ghosts.size();i++) {
    ...
    if (effective_tool == TIMELINE_TOOL_SLIP) {
      ...
      if (g.transition != nullptr
          && g.transition == hitvidio::ActiveSequence->clips.at(g.clip)->opening_transition) {
        g.clip_in = g.old_clip_in + frame_diff;
      }
  ...
  if (panel_timeline->importing) {
    QToolTip::showText(mapToGlobal(mouse_pos), frame_to_timecode(earliest_in_point, hitvidio::CurrentConfig.timecode_view, hitvidio::ActiveSequence->frame_rate));
  } else {
    QString tip = ((frame_diff < 0) ? "-" : "+") + frame_to_timecode(qAbs(frame_diff), hitvidio::CurrentConfig.timecode_view, hitvidio::ActiveSequence->frame_rate);
    if (panel_timeline->trim_target > -1) {
      ...
      if (g != nullptr) {
        ...
        tip += frame_to_timecode(len, hitvidio::CurrentConfig.timecode_view, hitvidio::ActiveSequence->frame_rate);
      }
    }
  ...
  if (hitvidio::ActiveSequence != nullptr) {
    ...
    if (current_tool_shows_cursor()) {
      panel_timeline->snap_to_timeline(&panel_timeline->cursor_frame,
                                       ...
                                       !hitvidio::CurrentConfig.edit_tool_also_seeks || !panel_timeline->selecting,
    ...
    if (panel_timeline->selecting) {
      ...
      if (hitvidio::ActiveSequence->selections.size() != selection_count) {
        hitvidio::ActiveSequence->selections.resize(selection_count);
      }
      ...
      if (hitvidio::CurrentConfig.edit_tool_selects_links) {
        ...
        for (int j=0;j<hitvidio::ActiveSequence->clips.size();j++) {
          ...
          if (c != nullptr && c->IsSelected(false)) {
            ...
            for (int k=0;k<c->linked.size();k++) {
              ...
              ClipPtr link = hitvidio::ActiveSequence->clips.at(c->linked.at(k));
              ...
              if (!(link->track() >= minimum_selection_track
                ...
                hitvidio::ActiveSequence->selections.append(link_sel);
      ...
      if (track_resizing) {
        ...
        new_height = qMax(new_height, hitvidio::timeline::kTrackMinHeight);
          ...
          for (int j=0;j<hitvidio::ActiveSequence->clips.size();j++) {
            ...
            if (c != nullptr) {
              ...
              for (int i=0;i<ghost_arr_size;i++) {
                ...
                ClipPtr ghost_clip = hitvidio::ActiveSequence->clips.at(g.clip);
        ...
        if (panel_timeline->tool == TIMELINE_TOOL_RIPPLE) {
          ...
          for (int i=0;i<panel_timeline->ghosts.size();i++) {
            ClipPtr c = hitvidio::ActiveSequence->clips.at(panel_timeline->ghosts.at(i).clip);
            if (panel_timeline->trim_type == TRIM_IN) {
        ...
        // store selections
        selection_command = new SetSelectionsCommand(hitvidio::ActiveSequence.get());
        selection_command->old_data = hitvidio::ActiveSequence->selections;
        ...
        for (int i=0;i<track_size;i++) {
          ...
          if (clip_index > -1) {
            ...
            for (int j=0;j<clip->linked.size();j++) {
              ...
              ClipPtr link = hitvidio::ActiveSequence->clips.at(clip->linked.at(j));
      ...
      if (panel_timeline->rect_select_proc) {
        ...
        for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
          ClipPtr clip = hitvidio::ActiveSequence->clips.at(i);
          if (clip != nullptr &&
            ...
            if (!alt) {
              for (int j=0;j<clip->linked.size();j++) {
                session_clips.append(hitvidio::ActiveSequence->clips.at(clip->linked.at(j)));
              }
            }
        ...
        hitvidio::ActiveSequence->selections.resize(selected_clips.size() + panel_timeline->selection_offset);
        for (int i=0;i<selected_clips.size();i++) {
          Selection& s = hitvidio::ActiveSequence->selections[i+panel_timeline->selection_offset];
      ...
      if (found) {
        ...
        if (panel_timeline->trim_type == TRIM_IN) { // if we're trimming an IN point
          setCursor(panel_timeline->tool == TIMELINE_TOOL_RIPPLE ? hitvidio::cursor::LeftRipple : hitvidio::cursor::LeftTrim);
        } else { // if we're trimming an OUT point
          setCursor(panel_timeline->tool == TIMELINE_TOOL_RIPPLE ? hitvidio::cursor::RightRipple : hitvidio::cursor::RightTrim);
        }
        ...
        if (mouse_pos > track_y_edge - test_range
          ...
          if (cursor_contains_clip
              || (hitvidio::CurrentConfig.show_track_lines
      ...
      if (getClipIndexFromCoords(panel_timeline->cursor_frame, panel_timeline->cursor_track) > -1) {
        setCursor(hitvidio::cursor::Slip);
      } else {
        unsetCursor();
      }
      ...
      if (panel_timeline->transition_tool_init) {
        ...
        if (panel_timeline->transition_tool_proc) {
          ...
          ClipPtr c = hitvidio::ActiveSequence->clips.at(primary);
        ...
        if (mouse_clip > -1) {
          ...
          ClipPtr c = hitvidio::ActiveSequence->clips.at(mouse_clip);
...
void draw_waveform(ClipPtr clip, const FootageStream* ms, long media_length, QPainter *p, const QRect& clip_rect, int waveform_start, int waveform_limit, double zoom) {
  ...
  for (int i=waveform_start;i<waveform_limit;i++) {
    ...
    for (int j=0;j<ms->audio_channels;j++) {
      int mid = (hitvidio::CurrentConfig.rectified_waveforms) ? clip_rect.top()+channel_height*(j+1) : clip_rect.top()+channel_height*j+(channel_height/2);
      ...
      if ((offset_range_max + 1) < ms->audio_preview.size()) {
        ...
        // draw waveforms
        if (hitvidio::CurrentConfig.rectified_waveforms)  {
...
void draw_transition(QPainter& p, ClipPtr c, const QRect& clip_rect, QRect& text_rect, int transition_type) {
  ...
  if (t != nullptr) {
    ...
    QRect transition_text_rect(transition_rect.x() + hitvidio::timeline::kClipTextPadding, transition_rect.y() + hitvidio::timeline::kClipTextPadding, transition_rect.width() - hitvidio::timeline::kClipTextPadding, transition_rect.height() - hitvidio::timeline::kClipTextPadding);
...
void TimelineWidget::paintEvent(QPaintEvent*) {
  ...
  if (hitvidio::ActiveSequence != nullptr) {
    ...
    int panel_height = hitvidio::timeline::kTrackDefaultHeight;
    ...
    for (int i=0;i<hitvidio::ActiveSequence->clips.size();i++) {
      ...
      if (clip != nullptr && is_track_visible(clip->track())) {
        ...
        QRect text_rect(clip_rect.left() + hitvidio::timeline::kClipTextPadding, clip_rect.top() + hitvidio::timeline::kClipTextPadding, clip_rect.width() - hitvidio::timeline::kClipTextPadding - 1, clip_rect.height() - hitvidio::timeline::kClipTextPadding - 1);
        if (clip_rect.left() < width() && clip_rect.right() >= 0 && clip_rect.top() < height() && clip_rect.bottom() >= 0) {
          ...
          if (clip->media() != nullptr && clip->media()->get_type() == MEDIA_TYPE_FOOTAGE) {
            ...
            if (ms == nullptr) {
              ...
              int triangle_size = hitvidio::timeline::kTrackMinHeight >> 2;
              ...
              if (clip->track() < 0) {
                // draw thumbnail
                int thumb_y = p.fontMetrics().height()+hitvidio::timeline::kClipTextPadding+hitvidio::timeline::kClipTextPadding;
              ...
              } else if (clip_rect.height() > hitvidio::timeline::kTrackMinHeight) {
                // draw waveform
          ...
          if (text_rect.width() > MAX_TEXT_WIDTH && text_rect.right() > 0 && text_rect.left() < width()) {
            ...
            if (clip->linked.size() > 0) {
              int underline_y = hitvidio::timeline::kClipTextPadding + p.fontMetrics().height() + clip_rect.top();
    ...
    // Draw track lines
    if (hitvidio::CurrentConfig.show_track_lines) {
      p.setPen(QColor(0, 0, 0, 96));
      audio_track_limit++;
    ...
    if (!panel_timeline->ghosts.isEmpty()) {
      ...
      for (int i=0;i<panel_timeline->ghosts.size();i++) {
        ...
        if (is_track_visible(g.track)) {
          ...
          for (int j=0;j<hitvidio::timeline::kGhostThickness;j++) {
      ...
      if (panel_timeline->move_insert && !insert_points.isEmpty()) {
        ...
        int tri_size = hitvidio::timeline::kTrackMinHeight>>2;
    ...
    // Draw playhead
    p.setPen(Qt::red);
    int playhead_x = panel_timeline->getTimelineScreenPointFromFrame(hitvidio::ActiveSequence->playhead);
    p.drawLine(playhead_x, rect().top(), playhead_x, rect().bottom());

    // Draw single frame highlight
    int playhead_frame_width = panel_timeline->getTimelineScreenPointFromFrame(hitvidio::ActiveSequence->playhead+1) - playhead_x;
...
int TimelineWidget::getTrackFromScreenPoint(int y) {
  ...
  while (true) {
    ...
    if (hitvidio::CurrentConfig.show_track_lines) track_height++;
    if (y < 0) {
...
int TimelineWidget::getScreenPointFromTrack(int track) {
  ...
  for (int i=start;i!=track;i+=interval) {
    ...
    if (hitvidio::CurrentConfig.show_track_lines) point++;
  }
