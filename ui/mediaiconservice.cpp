...
std::unique_ptr<MediaIconService> hitvidio::media_icon_service;
...
void MediaIconService::SetMediaIcon(Media *media, int icon_type) {
  ...
  case ICON_TYPE_VIDEO:
    hitvidio::project_model.set_icon(media, hitvidio::icon::MediaVideo);
    break;
  case ICON_TYPE_AUDIO:
    hitvidio::project_model.set_icon(media, hitvidio::icon::MediaAudio);
    break;
  case ICON_TYPE_IMAGE:
    hitvidio::project_model.set_icon(media, hitvidio::icon::MediaImage);
    break;
  ...
  case ICON_TYPE_ERROR:
    hitvidio::project_model.set_icon(media, hitvidio::icon::MediaError);
    break;
  }
...
void MediaIconService::AnimationUpdate() {
  ...
  for (int i=0;i<throbber_items_.size();i++) {
    hitvidio::project_model.set_icon(throbber_items_.at(i), throbber_ico);
  }
