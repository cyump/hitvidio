...
void TimelineHeader::set_in_point(long new_in) {
  ...
  hitvidio::UndoStack.push(new SetTimelineInOutCommand(viewer->seq.get(), true, new_in, new_out));
  update_parents();
}
...
void TimelineHeader::mouseReleaseEvent(QMouseEvent*) {
  if (viewer->seq != nullptr) {
    dragging = false;
    if (resizing_workarea) {
      hitvidio::UndoStack.push(new SetTimelineInOutCommand(viewer->seq.get(), true, temp_workarea_in, temp_workarea_out));
  ...
  if (viewer->seq != nullptr && zoom > 0) {
    ...
    while (true) {
      ...
      if (text_enabled && lineX-textWidth > lastTextBoundary) {
        timecode = frame_to_timecode(frame + in_visible, hitvidio::CurrentConfig.timecode_view, viewer->seq->frame_rate);
        ...
        if (hitvidio::CurrentConfig.center_timeline_timecodes) {
          text_x -= textWidth;
        } else {
      ...
      if (lineX > lastLineX+LINE_MIN_PADDING) {
        if (draw_text) {
          p.setPen(hitvidio::styling::GetIconColor());
          p.drawText(QRect(text_x, 0, fullTextWidth, yoff), timecode);
        }

        // draw line markers
        p.setPen(Qt::gray);
        p.drawLine(lineX, (!hitvidio::CurrentConfig.center_timeline_timecodes && draw_text) ? 0 : yoff, lineX, height());
...
void TimelineHeader::show_context_menu(const QPoint &pos) {
  ...
  hitvidio::MenuHelper.make_inout_menu(&menu);

  menu.addSeparator();

  QAction* center_timecodes = menu.addAction(tr("Center Timecodes"), &hitvidio::MenuHelper, SLOT(toggle_bool_action()));
  center_timecodes->setCheckable(true);
  center_timecodes->setChecked(hitvidio::CurrentConfig.center_timeline_timecodes);
  center_timecodes->setData(reinterpret_cast<quintptr>(&hitvidio::CurrentConfig.center_timeline_timecodes));
