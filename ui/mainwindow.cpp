...
MainWindow* hitvidio::MainWindow;
...
void MainWindow::setup_layout(bool reset) {
      ...
      while (!stream.atEnd()) {
        ...
        if (stream.name() == "panels" && stream.isStartElement()) {
          ...
          if (panel_name.isEmpty()) {
            ...
            for (int i=0;i<hitvidio::panels.size();i++) {

              Panel* panel = hitvidio::panels.at(i);
  ...
  if (reset) {
    ...
    for (int i=0;i<hitvidio::panels.size();i++) {
      removeDockWidget(hitvidio::panels.at(i));
    }
...
MainWindow::MainWindow(QWidget *parent) :
  ...
  first_show(true)
{
  hitvidio::cursor::Initialize();

  open_debug_file();

  hitvidio::DebugDialog = new DebugDialog(this);

  hitvidio::MainWindow = this;
  ...
  if (!data_dir.isEmpty()) {
    ...
    if (dir.exists()) {
      ...
      QFile f(hitvidio::Global->get_recent_project_list_file());
  ...
  if (!config_path.isEmpty()) {
    ...
    if (QFileInfo::exists(config_fn)) {
      hitvidio::CurrentConfig.load(config_fn);
    }
  }

  Restyle();

  hitvidio::icon::Initialize();
  ...
  statusBar->showMessage(tr("Welcome to %1").arg(hitvidio::AppName));
  setStatusBar(statusBar);

  hitvidio::Global->check_for_autorecovery_file();

  // lock panels if the config says so
  set_panels_locked(hitvidio::CurrentConfig.locked_panels);

  // set up output audio device
  init_audio();

  // start omnipotent proxy generator process
  hitvidio::proxy_generator.start();

  // load preferred language from file
  hitvidio::Global->load_translation_from_config();
...
{
  // Set up UI style
  if (!hitvidio::styling::UseNativeUI()) {
    qApp->setStyle(QStyleFactory::create("Fusion"));

    // Set up whether to load custom CSS or default CSS+palette
    if (!hitvidio::CurrentConfig.css_path.isEmpty()
        && load_css_from_file(hitvidio::CurrentConfig.css_path)) {
      ...
      if (hitvidio::CurrentConfig.style == hitvidio::styling::kHitvidioDefaultLight) {
...
void MainWindow::editMenu_About_To_Be_Shown() {
  undo_action->setEnabled(hitvidio::UndoStack.canUndo());
  redo_action->setEnabled(hitvidio::UndoStack.canRedo());
}
...
void MainWindow::setup_menus() {
  ...
  if (hitvidio::CurrentConfig.use_native_menu_styling) {
    HitvidioGlobal::SetNativeStyling(menuBar);
  }

  setMenuBar(menuBar);

  hitvidio::MenuHelper.InitializeSharedMenus();
  ...
  hitvidio::MenuHelper.make_new_menu(new_menu);

  open_project = MenuHelper::create_menu_action(file_menu, "openproj", hitvidio::Global.get(), SLOT(OpenProject()), QKeySequence("Ctrl+O"));
  ...
  save_project = MenuHelper::create_menu_action(file_menu, "saveproj", hitvidio::Global.get(), SLOT(save_project()), QKeySequence("Ctrl+S"));

  save_project_as = MenuHelper::create_menu_action(file_menu, "saveprojas", hitvidio::Global.get(), SLOT(save_project_as()), QKeySequence("Ctrl+Shift+S"));
  ...
  export_action = MenuHelper::create_menu_action(file_menu, "export", hitvidio::Global.get(), SLOT(open_export_dialog()), QKeySequence("Ctrl+M"));
  ...
  undo_action = MenuHelper::create_menu_action(edit_menu, "undo", hitvidio::Global.get(), SLOT(undo()), QKeySequence("Ctrl+Z"));
  redo_action = MenuHelper::create_menu_action(edit_menu, "redo", hitvidio::Global.get(), SLOT(redo()), QKeySequence("Ctrl+Shift+Z"));

  edit_menu->addSeparator();

  hitvidio::MenuHelper.make_edit_functions_menu(edit_menu);

  edit_menu->addSeparator();

  select_all_action = MenuHelper::create_menu_action(edit_menu, "selectall", &hitvidio::FocusFilter, SLOT(select_all()), QKeySequence("Ctrl+A"));
  ...
  hitvidio::MenuHelper.make_clip_functions_menu(edit_menu);
  ...
  hitvidio::MenuHelper.make_inout_menu(edit_menu);
  ...
  setedit_marker_ = MenuHelper::create_menu_action(edit_menu, "marker", &hitvidio::FocusFilter, SLOT(set_marker()), QKeySequence("M"));

  // INITIALIZE VIEW MENU

  view_menu = MenuHelper::create_submenu(menuBar, this, SLOT(viewMenu_About_To_Be_Shown()));

  zoom_in_ = MenuHelper::create_menu_action(view_menu, "zoomin", &hitvidio::FocusFilter, SLOT(zoom_in()), QKeySequence("="));
  zoom_out_ = MenuHelper::create_menu_action(view_menu, "zoomout", &hitvidio::FocusFilter, SLOT(zoom_out()), QKeySequence("-"));
  ...
  track_lines = MenuHelper::create_menu_action(view_menu, "tracklines", &hitvidio::MenuHelper, SLOT(toggle_bool_action()));
  track_lines->setCheckable(true);
  track_lines->setData(reinterpret_cast<quintptr>(&hitvidio::CurrentConfig.show_track_lines));

  rectified_waveforms = MenuHelper::create_menu_action(view_menu, "rectifiedwaveforms", &hitvidio::MenuHelper, SLOT(toggle_bool_action()));
  rectified_waveforms->setCheckable(true);
  rectified_waveforms->setData(reinterpret_cast<quintptr>(&hitvidio::CurrentConfig.rectified_waveforms));

  view_menu->addSeparator();

  QActionGroup* frame_view_mode_group = new QActionGroup(this);

  frames_action = MenuHelper::create_menu_action(view_menu, "modeframes", &hitvidio::MenuHelper, SLOT(set_timecode_view()));
  frames_action->setData(hitvidio::kTimecodeFrames);
  frames_action->setCheckable(true);
  frame_view_mode_group->addAction(frames_action);

  drop_frame_action = MenuHelper::create_menu_action(view_menu, "modedropframe", &hitvidio::MenuHelper, SLOT(set_timecode_view()));
  drop_frame_action->setData(hitvidio::kTimecodeDrop);
  drop_frame_action->setCheckable(true);
  frame_view_mode_group->addAction(drop_frame_action);

  nondrop_frame_action = MenuHelper::create_menu_action(view_menu, "modenondropframe", &hitvidio::MenuHelper, SLOT(set_timecode_view()));
  nondrop_frame_action->setData(hitvidio::kTimecodeNonDrop);
  nondrop_frame_action->setCheckable(true);
  frame_view_mode_group->addAction(nondrop_frame_action);

  milliseconds_action = MenuHelper::create_menu_action(view_menu, "milliseconds", &hitvidio::MenuHelper, SLOT(set_timecode_view()));
  milliseconds_action->setData(hitvidio::kTimecodeMilliseconds);
  ...
  title_safe_off = MenuHelper::create_menu_action(title_safe_area_menu, "titlesafeoff", &hitvidio::MenuHelper, SLOT(set_titlesafe_from_menu()));
  title_safe_off->setCheckable(true);
  title_safe_off->setData(qSNaN());
  title_safe_group->addAction(title_safe_off);

  title_safe_default = MenuHelper::create_menu_action(title_safe_area_menu, "titlesafedefault", &hitvidio::MenuHelper, SLOT(set_titlesafe_from_menu()));
  title_safe_default->setCheckable(true);
  title_safe_default->setData(0.0);
  title_safe_group->addAction(title_safe_default);

  title_safe_43 = MenuHelper::create_menu_action(title_safe_area_menu, "titlesafe43", &hitvidio::MenuHelper, SLOT(set_titlesafe_from_menu()));
  title_safe_43->setCheckable(true);
  title_safe_43->setData(4.0/3.0);
  title_safe_group->addAction(title_safe_43);

  title_safe_169 = MenuHelper::create_menu_action(title_safe_area_menu, "titlesafe169", &hitvidio::MenuHelper, SLOT(set_titlesafe_from_menu()));
  title_safe_169->setCheckable(true);
  title_safe_169->setData(16.0/9.0);
  title_safe_group->addAction(title_safe_169);

  title_safe_custom = MenuHelper::create_menu_action(title_safe_area_menu, "titlesafecustom", &hitvidio::MenuHelper, SLOT(set_titlesafe_from_menu()));
  ...
  full_screen_viewer_ = MenuHelper::create_menu_action(view_menu, "fullscreenviewer", &hitvidio::FocusFilter, SLOT(set_viewer_fullscreen()));

  // INITIALIZE PLAYBACK MENU

  playback_menu = MenuHelper::create_submenu(menuBar, this, SLOT(playbackMenu_About_To_Be_Shown()));

  go_to_start_ = MenuHelper::create_menu_action(playback_menu, "gotostart", &hitvidio::FocusFilter, SLOT(go_to_start()), QKeySequence("Home"));
  previous_frame_ = MenuHelper::create_menu_action(playback_menu, "prevframe", &hitvidio::FocusFilter, SLOT(prev_frame()), QKeySequence("Left"));
  playpause_ = MenuHelper::create_menu_action(playback_menu, "playpause", &hitvidio::FocusFilter, SLOT(playpause()), QKeySequence("Space"));
  play_in_to_out_ = MenuHelper::create_menu_action(playback_menu, "playintoout", &hitvidio::FocusFilter, SLOT(play_in_to_out()), QKeySequence("Shift+Space"));
  next_frame_ = MenuHelper::create_menu_action(playback_menu, "nextframe", &hitvidio::FocusFilter, SLOT(next_frame()), QKeySequence("Right"));
  go_to_end_ = MenuHelper::create_menu_action(playback_menu, "gotoend", &hitvidio::FocusFilter, SLOT(go_to_end()), QKeySequence("End"));
  ...
  go_to_in_point_ = MenuHelper::create_menu_action(playback_menu, "gotoin", &hitvidio::FocusFilter, SLOT(go_to_in()), QKeySequence("Shift+I"));
  go_to_out_point_ = MenuHelper::create_menu_action(playback_menu, "gotoout", &hitvidio::FocusFilter, SLOT(go_to_out()), QKeySequence("Shift+O"));

  playback_menu->addSeparator();

  shuttle_left_ = MenuHelper::create_menu_action(playback_menu, "decspeed", &hitvidio::FocusFilter, SLOT(decrease_speed()), QKeySequence("J"));
  shuttle_stop_ = MenuHelper::create_menu_action(playback_menu, "pause", &hitvidio::FocusFilter, SLOT(pause()), QKeySequence("K"));
  shuttle_right_ = MenuHelper::create_menu_action(playback_menu, "incspeed", &hitvidio::FocusFilter, SLOT(increase_speed()), QKeySequence("L"));

  playback_menu->addSeparator();

  loop_action_ = MenuHelper::create_menu_action(playback_menu, "loop", &hitvidio::MenuHelper, SLOT(toggle_bool_action()));
  loop_action_->setCheckable(true);
  loop_action_->setData(reinterpret_cast<quintptr>(&hitvidio::CurrentConfig.loop));
  ...
  pointer_tool_action = MenuHelper::create_menu_action(tools_menu, "pointertool", &hitvidio::MenuHelper, SLOT(menu_click_button()), QKeySequence("V"));
  ...
  edit_tool_action = MenuHelper::create_menu_action(tools_menu, "edittool", &hitvidio::MenuHelper, SLOT(menu_click_button()), QKeySequence("X"));
  ...
  ripple_tool_action = MenuHelper::create_menu_action(tools_menu, "rippletool", &hitvidio::MenuHelper, SLOT(menu_click_button()), QKeySequence("B"));
  ...
  razor_tool_action = MenuHelper::create_menu_action(tools_menu, "razortool", &hitvidio::MenuHelper, SLOT(menu_click_button()), QKeySequence("C"));
  ...
  slip_tool_action = MenuHelper::create_menu_action(tools_menu, "sliptool", &hitvidio::MenuHelper, SLOT(menu_click_button()), QKeySequence("Y"));
  ...
  slide_tool_action = MenuHelper::create_menu_action(tools_menu, "slidetool", &hitvidio::MenuHelper, SLOT(menu_click_button()), QKeySequence("U"));
  ...
  hand_tool_action = MenuHelper::create_menu_action(tools_menu, "handtool", &hitvidio::MenuHelper, SLOT(menu_click_button()), QKeySequence("H"));
  ...
  transition_tool_action = MenuHelper::create_menu_action(tools_menu, "transitiontool", &hitvidio::MenuHelper, SLOT(menu_click_button()), QKeySequence("T"));
  ...
  snap_toggle = MenuHelper::create_menu_action(tools_menu, "snapping", &hitvidio::MenuHelper, SLOT(menu_click_button()), QKeySequence("S"));
  snap_toggle->setCheckable(true);
  snap_toggle->setData(reinterpret_cast<quintptr>(panel_timeline->snappingButton));

  tools_menu->addSeparator();

  autocut_silence_ = MenuHelper::create_menu_action(tools_menu, "autocutsilence", hitvidio::Global.get(), SLOT(open_autocut_silence_dialog()));

  tools_menu->addSeparator();

  QActionGroup* autoscroll_group = new QActionGroup(this);

  no_autoscroll = MenuHelper::create_menu_action(tools_menu, "autoscrollno", &hitvidio::MenuHelper, SLOT(set_autoscroll()));
  no_autoscroll->setData(hitvidio::AUTOSCROLL_NO_SCROLL);
  no_autoscroll->setCheckable(true);
  autoscroll_group->addAction(no_autoscroll);

  page_autoscroll = MenuHelper::create_menu_action(tools_menu, "autoscrollpage", &hitvidio::MenuHelper, SLOT(set_autoscroll()));
  page_autoscroll->setData(hitvidio::AUTOSCROLL_PAGE_SCROLL);
  page_autoscroll->setCheckable(true);
  autoscroll_group->addAction(page_autoscroll);

  smooth_autoscroll = MenuHelper::create_menu_action(tools_menu, "autoscrollsmooth", &hitvidio::MenuHelper, SLOT(set_autoscroll()));
  smooth_autoscroll->setData(hitvidio::AUTOSCROLL_SMOOTH_SCROLL);
  smooth_autoscroll->setCheckable(true);
  autoscroll_group->addAction(smooth_autoscroll);

  tools_menu->addSeparator();

  preferences_action_ = MenuHelper::create_menu_action(tools_menu, "prefs", hitvidio::Global.get(), SLOT(open_preferences()), QKeySequence("Ctrl+,"));

#ifdef QT_DEBUG
  clear_undo_action_ = MenuHelper::create_menu_action(tools_menu, "clearundo", hitvidio::Global.get(), SLOT(clear_undo_stack()));
#endif
  ...
  action_search_ = MenuHelper::create_menu_action(help_menu, "actionsearch", hitvidio::Global.get(), SLOT(open_action_search()), QKeySequence("/"));

  help_menu->addSeparator();

  debug_log_ = MenuHelper::create_menu_action(help_menu, "debuglog", hitvidio::Global.get(), SLOT(open_debug_log()));

  help_menu->addSeparator();

  about_action_ = MenuHelper::create_menu_action(help_menu, "about", hitvidio::Global.get(), SLOT(open_about_dialog()));
...
void MainWindow::Retranslate()
{
  ...
  for (int i=0;i<hitvidio::panels.size();i++) {
    hitvidio::panels.at(i)->Retranslate();
  }
  hitvidio::MenuHelper.Retranslate();

  updateTitle();
}

void MainWindow::updateTitle() {
  setWindowTitle(QString("%1 - %2[*]").arg(hitvidio::AppName,
                                           (hitvidio::ActiveProjectFilename.isEmpty()) ?
                                             tr("<untitled>") : hitvidio::ActiveProjectFilename)
                 );
}

void MainWindow::closeEvent(QCloseEvent *e) {
  if (hitvidio::Global->can_close_project()) {
    // stop proxy generator thread
    hitvidio::proxy_generator.cancel();
    ...
    if (!config_path.isEmpty()) {
      ...
      // save settings
      hitvidio::CurrentConfig.save(config_fn);
      ...
      if (panel_config.open(QFile::WriteOnly)) {
        ...
        for (int i=0;i<hitvidio::panels.size();i++) {
          QByteArray layout_data = hitvidio::panels.at(i)->SaveLayoutState();

          if (!layout_data.isEmpty()) {

            // layout data is matched with the panel's objectName(), which we can't do if the panel has no name
            const QString& panel_name = hitvidio::panels.at(i)->objectName();
            if (panel_name.isEmpty()) {
...
void MainWindow::maximize_panel() {
  ...
  if (temp_panel_state.isEmpty()) {
    ...
    if (focused_panel != nullptr) {
      ...
      for (int i=0;i<hitvidio::panels.size();i++) {
        if (hitvidio::panels.at(i) != focused_panel) {
          // hide the panel
          hitvidio::panels.at(i)->setVisible(false);

          // set it to floating
          hitvidio::panels.at(i)->setFloating(true);
        }
      }
    }
  } else {
...
void MainWindow::playbackMenu_About_To_Be_Shown() {
  hitvidio::MenuHelper.set_bool_action_checked(loop_action_);
}

void MainWindow::viewMenu_About_To_Be_Shown() {
  hitvidio::MenuHelper.set_bool_action_checked(track_lines);

  hitvidio::MenuHelper.set_bool_action_checked(rectified_waveforms);

  hitvidio::MenuHelper.set_int_action_checked(frames_action, hitvidio::CurrentConfig.timecode_view);
  hitvidio::MenuHelper.set_int_action_checked(drop_frame_action, hitvidio::CurrentConfig.timecode_view);
  hitvidio::MenuHelper.set_int_action_checked(nondrop_frame_action, hitvidio::CurrentConfig.timecode_view);
  hitvidio::MenuHelper.set_int_action_checked(milliseconds_action, hitvidio::CurrentConfig.timecode_view);

  title_safe_off->setChecked(!hitvidio::CurrentConfig.show_title_safe_area);
  title_safe_default->setChecked(hitvidio::CurrentConfig.show_title_safe_area
                                 && !hitvidio::CurrentConfig.use_custom_title_safe_ratio);
  title_safe_43->setChecked(hitvidio::CurrentConfig.show_title_safe_area
                            && hitvidio::CurrentConfig.use_custom_title_safe_ratio
                            && qFuzzyCompare(hitvidio::CurrentConfig.custom_title_safe_ratio, title_safe_43->data().toDouble()));
  title_safe_169->setChecked(hitvidio::CurrentConfig.show_title_safe_area
                             && hitvidio::CurrentConfig.use_custom_title_safe_ratio
                             && qFuzzyCompare(hitvidio::CurrentConfig.custom_title_safe_ratio, title_safe_169->data().toDouble()));
  title_safe_custom->setChecked(hitvidio::CurrentConfig.show_title_safe_area
                                && hitvidio::CurrentConfig.use_custom_title_safe_ratio
...
void MainWindow::toolMenu_About_To_Be_Shown() {
  hitvidio::MenuHelper.set_button_action_checked(pointer_tool_action);
  hitvidio::MenuHelper.set_button_action_checked(edit_tool_action);
  hitvidio::MenuHelper.set_button_action_checked(ripple_tool_action);
  hitvidio::MenuHelper.set_button_action_checked(razor_tool_action);
  hitvidio::MenuHelper.set_button_action_checked(slip_tool_action);
  hitvidio::MenuHelper.set_button_action_checked(slide_tool_action);
  hitvidio::MenuHelper.set_button_action_checked(hand_tool_action);
  hitvidio::MenuHelper.set_button_action_checked(transition_tool_action);
  hitvidio::MenuHelper.set_button_action_checked(snap_toggle);

  hitvidio::MenuHelper.set_int_action_checked(no_autoscroll, hitvidio::CurrentConfig.autoscroll);
  hitvidio::MenuHelper.set_int_action_checked(page_autoscroll, hitvidio::CurrentConfig.autoscroll);
  hitvidio::MenuHelper.set_int_action_checked(smooth_autoscroll, hitvidio::CurrentConfig.autoscroll);
}
...
void MainWindow::set_panels_locked(bool locked)
{
  ...
  hitvidio::CurrentConfig.locked_panels = locked;
}
...
void MainWindow::fileMenu_About_To_Be_Shown() {
  if (recent_projects.size() > 0) {
    ...
    for (int i=0;i<recent_projects.size();i++) {
      ...
      action->setData(i);
      connect(action, SIGNAL(triggered()), &hitvidio::MenuHelper, SLOT(open_recent_from_menu()));
    }
