...
void ViewerWidget::frame_update() {
  if (viewer->seq != nullptr) {
    ...
    // render the audio
    hitvidio::rendering::compose_audio(viewer, viewer->seq.get(), viewer->get_playback_speed(), false);
  }
}
...
void ViewerWidget::mouseMoveEvent(QMouseEvent* event) {
  ...
  if (dragging) {
    if (waveform) {
      ...
      if (gizmos == nullptr) {
        viewer->initiate_drag(hitvidio::timeline::kImportBoth);
        dragging = false;
      } else {
...
void ViewerWidget::draw_title_safe_area() {
  ...
  if (hitvidio::CurrentConfig.use_custom_title_safe_ratio && hitvidio::CurrentConfig.custom_title_safe_ratio > 0) {
    if (hitvidio::CurrentConfig.custom_title_safe_ratio > viewportAr) {
      halfHeight = (hitvidio::CurrentConfig.custom_title_safe_ratio/viewportAr)*0.5;
    } else {
      halfWidth = (viewportAr/hitvidio::CurrentConfig.custom_title_safe_ratio)*0.5;
    }
  }
...
void ViewerWidget::paintGL() {
  if (waveform) {
    ...
    if (hitvidio::CurrentConfig.show_title_safe_area) {
