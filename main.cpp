...
int main(int argc, char *argv[]) {
  Hitvidio::Global = std::unique_ptr<HitvidioGlobal>(new HitvidioGlobal);
      ...
      if (argv[i][0] == '-') {
        if (!strcmp(argv[i], "--version") || !strcmp(argv[i], "-v")) {
...
#endif
          printf("%s\n", Hitvidio::AppName.toUtf8().constData());
          return 0;
          ...
          printf("Usage: %s [options] [filename]\n\n"
                 ...
                 "\tHitvidio_EFFECTS_PATH\tSpecify a path to search for GLSL shader effects\n"
                 "\tFREI0R_PATH\t\tSpecify a path to search for Frei0r effects\n"
                 "\tHitvidio_LANG_PATH\t\tSpecify a path to search for translation files\n"
                 "\n", argv[0]);
          return 0;
        } else if (!strcmp(argv[i], "--fullscreen") || !strcmp(argv[i], "-f")) {
          launch_fullscreen = true;
        } else if (!strcmp(argv[i], "--disable-shaders")) {
          Hitvidio::CurrentRuntimeConfig.shaders_are_enabled = false;
        } else if (!strcmp(argv[i], "--no-debug")) {
          use_internal_logger = false;
        } else if (!strcmp(argv[i], "--disable-blend-modes")) {
          Hitvidio::CurrentRuntimeConfig.disable_blending = true;
        } else if (!strcmp(argv[i], "--translation")) {
          if (i + 1 < argc && argv[i + 1][0] != '-') {
            // load translation file
            Hitvidio::CurrentRuntimeConfig.external_translation_file = argv[i + 1];
  ...
  a.setWindowIcon(QIcon(":/icons/Hitvidio64.png"));

  // start media icon service (uses QPixmaps which require a QGuiApplication to have been created)
  Hitvidio::media_icon_service = std::unique_ptr<MediaIconService>(new MediaIconService());

  // set app name data
  QCoreApplication::setOrganizationName("Hitvidiovideoeditor.org");
  QCoreApplication::setOrganizationDomain("Hitvidiovideoeditor.org");
  QCoreApplication::setApplicationName("Hitvidio");

#if (QT_VERSION >= QT_VERSION_CHECK(5, 7, 0))
  QGuiApplication::setDesktopFileName("org.Hitvidiovideoeditor.Hitvidio");
#endif

  MainWindow w(nullptr);

  // multiply track height constants by the current DPI scale
  Hitvidio::timeline::MultiplyTrackSizesByDPI();

  // connect main window's first paint to global's init finished function
  QObject::connect(&w, SIGNAL(finished_first_paint()), Hitvidio::Global.get(), SLOT(finished_initialize()), Qt::QueuedConnection);

  if (!load_proj.isEmpty()) {
    Hitvidio::Global->load_project_on_launch(load_proj);
  }
