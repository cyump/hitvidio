...
Config hitvidio::CurrentConfig;
RuntimeConfig hitvidio::CurrentRuntimeConfig;
...
Config::Config()
  : show_track_lines(true),
    ...
    timecode_view(hitvidio::kTimecodeDrop),
    ...
    autoscroll(hitvidio::AUTOSCROLL_PAGE_SCROLL),
    audio_rate(48000),
    hover_focus(false),
    project_view_type(hitvidio::PROJECT_VIEW_TREE),
    set_name_with_marker(true),
    show_project_toolbar(true),
    previous_queue_size(3),
    previous_queue_type(hitvidio::FRAME_QUEUE_TYPE_FRAMES),
    upcoming_queue_size(0.5),
    upcoming_queue_type(hitvidio::FRAME_QUEUE_TYPE_SECONDS),
    loop(false),
    ...
    style(hitvidio::styling::kHitvidioDefaultDark),
    ...
    while (!stream.atEnd()) {
      ...
      if (stream.isStartElement()) {
        if (stream.name() == "ShowTrackLines") {
          ...
          stream.readNext();
          style = static_cast<hitvidio::styling::Style>(stream.text().toInt());
