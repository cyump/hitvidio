...
/**
 * @brief The Hitvidio Global class
 *
 * A resource for various global functions used throughout Hitvidio.
 */
class HitvidioGlobal : public QObject {
    Q_OBJECT
public:
    /**
     * @brief HitvidioGlobal Constructor
     *
     * Creates Hitvidio Global object. Also sets some default runtime settings and the application name.
     */
    HitvidioGlobal();

    /**
     * @brief Returns the file dialog filter used when interfacing with Hitvidio project files.
     *
     * @return The file filter string used by QFileDialog to limit the files shown to Hitvidio (*.ove) files.
     */
     ...
     * saving or loading functions in order to set which file to work with (HitvidioGlobal::open_project() and
     * HitvidioGlobal::save_project_as() do this automatically). Also updates the main window title to reflect the
     * project filename.
     *
     * @param s
     *
     * The URL of the project file to work with. Can be an empty string, in which case Hitvidio will treat the project
     ...
     *
     * Usually called on initialization. Checks if an auto-recovery file exists (meaning the last session of Hitvidio
     ...
     *
     * Some background functions shouldn't run while Hitvidio is exporting a video. This function will disable/enable them
     * as necessary.
     *
     * The current functions are as follows:
     * * Auto-recovery interval. Hitvidio saves an auto-recovery just before exporting anyway and seeing as the user
     * cannot make changes while rendering, there's no reason to continue saving auto-recovery files.
     * * Audio device playback. Hitvidio uses the same internal audio buffer for exporting as it does for playback, but
     * this buffer does not need to be forwarded to the output device when exporting.
     *
     * @param rendering
     *
     * **TRUE** if Hitvidio is about to export a video. **FALSE** if Hitvidio has finished exporting.
     */
     ...
     *
     * Called by main() if Hitvidio was called with a project file as a running argument. Sets up Hitvidio to load the
    ...
    /**
     * @brief (Re)load translation file from hitvidio::config
     */
...
public slots:
     ...
     *
     * Imports an Hitvidio project into the current project, effectively merging them.
     *
     * @param fn
     *
     ...
     * exiting Hitvidio, etc.) If the project has unsaved changes, this function asks the user whether they want to save or
    ...
    /**
     * @brief Open the About Hitvidio dialog.
     */
    ...
    /**
     * @brief Function called when Hitvidio has finished starting up
     *
     * Sets up some last things for Hitvidio that must be run after Hitvidio has completed initialization. If a project was
...
private:
     ...
     * @param autorecovery
     *
     * Whether this file is an autorecovery file. If it is, after the load Hitvidio will set the project URL to a new file
     * beside the original project file so that it does not overwrite the original and so that the user is not working
     * on the autorecovery project in Hitvidio's application data directory.
     */
     ...
     *
     * Loads data from an Hitvidio project file creating a LoadDialog to show visual information and a LoadThread to load
    ...
    /**
     * @brief File filter used for any file dialogs relating to Hitvidio project files.
     */
...
namespace hitvidio {
    /**
     * @brief Object resource for various global functions used throughout Hitvidio
     */
    extern std::unique_ptr<HitvidioGlobal> Global;
