...
namespace hitvidio {
   ...
   *
   * This constant is used to identify what version of Hitvidio a project file was saved with. Every project file
   * is saved with the current version number and the version is checked whenever an Hitvidio project is loaded to
  ...
  /**
   * @brief Minimum project version that this version of Hitvidio can open
   *
   * When loading a project, the project's version number is actually checked whether it is somewhere between
   * kSaveVersion and this value (inclusive). This is used if the current version of Hitvidio contains backwards
   ...
   *
   * Hitvidio currently supports recording mono or stereo and gives users the option of which mode to use when
   ...
   *
   * The Timeline in Hitvidio can automatically scroll to follow the playhead when the sequence is playing.
  ...
  enum AutoScrollMode {
    ...
    /** Smooth auto-scroll, Hitvidio will scroll to keep the playhead in the center of the screen at all times while
     * playing */
   ...
   *
   * Hitvidio keeps a "frame queue" in memory to allow smoother playback/seeking. In order to give users control over
 ...
 *
 * This struct handles any configuration that should persist between restarting Hitvidio. It contains several variables
...
struct Config {
   ...
   *
   * A '|' separated list of file extensions that Hitvidio will perform an image sequence test heuristic on when importing
   */
   ...
   *
   * **TRUE** if Hitvidio should display waveforms as "rectified". Rectified waveforms start at the bottom rather than
   * from the middle.
   */
  ...
  /**
   * @brief Enable dragging files outside Hitvidio directly into the Timeline
   *
   * **TRUE** if the Timeline should respond to files dropped from outside Hitvidio.
   */
   ...
   *
   * When recording audio within Hitvidio, use this mode/channel layout (e.g. mono or stereo).
   *
   ...
   *
   * **TRUE** if dropping a file from outside Hitvidio onto a media item in the Project panel should prompt the user
   ...
   * on, but Hitvidio also supports panels being considered "focused" if the mouse is hovered over them.
   *
   ...
   *
   * **TRUE** if Hitvidio should ask the user to name a marker when setting one. **FALSE** if markers should just be
   ...
   *
   * Hitvidio has an optional toolbar for the Project panel.
   *
   ...
   *
   * Hitvidio caches frames in memory to improve playback performance (see enum FrameQueueType documentation for more
   ...
   * @brief Loop
   *
   * If an in/out point are set on the Sequence (Sequence::using_workarea is **TRUE**), set this to **TRUE** if Hitvidio
   ...
   *
   * Hitvidio supports automatically selecting clips that the playhead is currently touching for a more efficient workflow.
   *
   ...
   *
   * Hitvidio uses a lot of OpenGL-based hardware acceleration for performance. Some older hardware has difficulty
   * supporting this functionality, so some of it has software-based (not hardware accelerated) fallbacks for these
   * users.
   *
   * **TRUE** if Hitvidio should prefer software fallbacks to hardware acceleration when they're available.
   */
   ...
   *
   * By default, Hitvidio shows timecodes in the TimelineHeader centered to the corresponding frame point. This may not
   * always be desirable as, for example, this forces the initial 00:00:00;00 timecode's left half to be cut off.
   * Hitvidio supports aligning the timecode to the right of the frame rather than the center to address this.
   */
   ...
   *
   * Sets the audio device Hitvidio should use to output audio to.
   *
   ...
   *
   * Sets the audio device Hitvidio should use to input audio from.
   *
   * Set to the name of the audio device or **EMPTY** to try using the default.
   */
   ...
   *
   * Sets the translation file to load to display Hitvidio in a different language.
   *
  ...
  /**
   * @brief Style to use when theming Hitvidio.
   *
   * Set to a member of hitvidio::styling::Style.
   */
  hitvidio::styling::Style style;
 ...
 *
 * This struct handles any configuration that's set as a command-line argument to Hitvidio, and shouldn't be persistent
 * between restarts of Hitvidio.
 */
