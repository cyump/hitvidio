...
ExportDialog::ExportDialog(QWidget *parent) :
  QDialog(parent)
{
  setWindowTitle(tr("Export \"%1\"").arg(hitvidio::ActiveSequence->name));
  setup_ui();

  rangeCombobox->setCurrentIndex(0);
  if (hitvidio::ActiveSequence->using_workarea) {
  ...
  widthSpinbox->setValue(hitvidio::ActiveSequence->width);
  heightSpinbox->setValue(hitvidio::ActiveSequence->height);
  samplingRateSpinbox->setValue(hitvidio::ActiveSequence->audio_frequency);
  framerateSpinbox->setValue(hitvidio::ActiveSequence->frame_rate);
...
void ExportDialog::StartExport() {
  ...
  if (!filename.isEmpty()) {
    ...
    params.end_frame = hitvidio::ActiveSequence->getEndFrame(); // entire sequence
    if (rangeCombobox->currentIndex() == 1) {
      params.start_frame = qMax(hitvidio::ActiveSequence->workarea_in, params.start_frame);
      params.end_frame = qMin(hitvidio::ActiveSequence->workarea_out, params.end_frame);
    }
    ...
    hitvidio::Global->set_rendering_state(true);

    // Close all currently open clips
    close_active_clips(hitvidio::ActiveSequence.get());

    hitvidio::Global->save_autorecovery_file();
...
void ExportDialog::comp_type_changed(int) {
  ...
  case COMPRESSION_TYPE_TARGETBR:
    ...
    videobitrateSpinbox->setValue(qMax(0.5, (double) qRound((0.01528 * hitvidio::ActiveSequence->height) - 4.5)));
    break;
