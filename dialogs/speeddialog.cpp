...
SpeedDialog::SpeedDialog(QWidget *parent, QVector<Clip*> clips) : QDialog(parent) {
  ...
  duration->SetFrameRate(hitvidio::ActiveSequence->frame_rate);
...
void set_speed(ComboAction* ca, Clip* c, double speed, bool ripple, long& ep, long& lr) {
  ...
  sel.track = c->track();
  hitvidio::ActiveSequence->selections.append(sel);
}
...
void SpeedDialog::accept() {
  ...
  SetSelectionsCommand* sel_command = new SetSelectionsCommand(hitvidio::ActiveSequence.get());
  sel_command->old_data = hitvidio::ActiveSequence->selections;
  ...
  sel_command->new_data = hitvidio::ActiveSequence->selections;
