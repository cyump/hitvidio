  ...
  QDialog(parent)
{
  ...
  setup_kbd_shortcuts(hitvidio::MainWindow->menuBar());

  // set up default sequence
  default_sequence.name = tr("Default Sequence");
  default_sequence.width = hitvidio::CurrentConfig.default_sequence_width;
  default_sequence.height = hitvidio::CurrentConfig.default_sequence_height;
  default_sequence.frame_rate = hitvidio::CurrentConfig.default_sequence_framerate;
  default_sequence.audio_frequency = hitvidio::CurrentConfig.default_sequence_audio_frequency;
  default_sequence.audio_layout = hitvidio::CurrentConfig.default_sequence_audio_channel_layout;
}
...
void PreferencesDialog::accept() {
  ...
  if (hitvidio::CurrentConfig.effect_textbox_lines != effect_textbox_lines_field->value()) {
    reload_effects = true;
  }
  ...
  // Check if any settings will require a restart of Hitvidio
  if (bool_requires_restart
      || hitvidio::CurrentConfig.thumbnail_resolution != thumbnail_res_spinbox->value()
      || hitvidio::CurrentConfig.waveform_resolution != waveform_res_spinbox->value()
      || hitvidio::CurrentConfig.css_path != custom_css_fn->text()
      || hitvidio::CurrentConfig.style != static_cast<hitvidio::styling::Style>(ui_style->currentData().toInt())) {

    // any changes to these settings will require a restart - ask the user if we should do one now or later

    int ret = QMessageBox::question(this,
                                    "Restart Required",
                                    "Some of the changed settings will require a restart of Hitvidio. Would you like "
                                    "to restart now?",
    ...
    // Selecting "No" will save the settings and not restart. They will become active next time Hitvidio opens.

  }

  // Audio settings may require the audio device to be re-initiated.
  if (hitvidio::CurrentConfig.preferred_audio_output != audio_output_devices->currentData().toString()
      || hitvidio::CurrentConfig.preferred_audio_input != audio_input_devices->currentData().toString()
      || hitvidio::CurrentConfig.audio_rate != audio_sample_rate->currentData().toInt()) {
    reinit_audio = true;
  }

  // see if the language file should be reloaded (not necessary if the app is restarting anyway)
  if (!restart_after_saving
      && hitvidio::CurrentConfig.language_file != language_combobox->currentData().toString()) {
    reload_language = true;
  }

  // save settings from UI to backend
  hitvidio::CurrentConfig.css_path = custom_css_fn->text();
  hitvidio::CurrentConfig.recording_mode = recordingComboBox->currentIndex() + 1;
  hitvidio::CurrentConfig.img_seq_formats = imgSeqFormatEdit->text();
  hitvidio::CurrentConfig.upcoming_queue_size = upcoming_queue_spinbox->value();
  hitvidio::CurrentConfig.upcoming_queue_type = upcoming_queue_type->currentIndex();
  hitvidio::CurrentConfig.previous_queue_size = previous_queue_spinbox->value();
  hitvidio::CurrentConfig.previous_queue_type = previous_queue_type->currentIndex();

  hitvidio::CurrentConfig.preferred_audio_output = audio_output_devices->currentData().toString();
  hitvidio::CurrentConfig.preferred_audio_input = audio_input_devices->currentData().toString();
  hitvidio::CurrentConfig.audio_rate = audio_sample_rate->currentData().toInt();

  hitvidio::CurrentConfig.effect_textbox_lines = effect_textbox_lines_field->value();
  hitvidio::CurrentConfig.language_file = language_combobox->currentData().toString();

  hitvidio::CurrentConfig.default_sequence_width = default_sequence.width;
  hitvidio::CurrentConfig.default_sequence_height = default_sequence.height;
  hitvidio::CurrentConfig.default_sequence_framerate = default_sequence.frame_rate;
  hitvidio::CurrentConfig.default_sequence_audio_frequency = default_sequence.audio_frequency;
  hitvidio::CurrentConfig.default_sequence_audio_channel_layout = default_sequence.audio_layout;
  ...
  if (hitvidio::CurrentConfig.thumbnail_resolution != thumbnail_res_spinbox->value()
      || hitvidio::CurrentConfig.waveform_resolution != waveform_res_spinbox->value()) {
    ...
    if (hitvidio::CurrentConfig.waveform_resolution != waveform_res_spinbox->value()) {
  ...
  if (restart_after_saving) {
    ...
    hitvidio::MainWindow->close();

    QProcess::startDetached(QApplication::applicationFilePath(), { hitvidio::ActiveProjectFilename });
  }
}
...
void PreferencesDialog::setup_ui() {
  ...
  for (int j=0;j<translation_paths.size();j++) {
    ...
    if (translation_dir.exists()) {
      ...
      for (int i=0;i<translation_files.size();i++) {
        ...
        if (hitvidio::CurrentConfig.language_file == locale_relative_path) {
  ...
  imgSeqFormatEdit->setText(hitvidio::CurrentConfig.img_seq_formats);
  ...
  thumbnail_res_spinbox->setValue(hitvidio::CurrentConfig.thumbnail_resolution);
  ...
  waveform_res_spinbox->setValue(hitvidio::CurrentConfig.waveform_resolution);
  ...
  AddBoolPair(use_software_fallbacks_checkbox, &hitvidio::CurrentConfig.use_software_fallback, true);
  ...
  AddBoolPair(add_default_effects_to_clips, &hitvidio::CurrentConfig.add_default_effects_to_clips);
  ...
  AddBoolPair(scroll_wheel_zooms, &hitvidio::CurrentConfig.scroll_zooms);
  ...
  AddBoolPair(invert_timeline_scroll_axes, &hitvidio::CurrentConfig.invert_timeline_scroll_axes);
  ...
  AddBoolPair(enable_drag_files_to_timeline, &hitvidio::CurrentConfig.enable_drag_files_to_timeline);
  ...
  AddBoolPair(autoscale_by_default, &hitvidio::CurrentConfig.autoscale_by_default);
  behavior_tab_layout->Add(autoscale_by_default);

  QCheckBox* enable_seek_to_import = new QCheckBox(tr("Auto-Seek to Imported Clips"));
  AddBoolPair(enable_seek_to_import, &hitvidio::CurrentConfig.enable_seek_to_import);
  behavior_tab_layout->Add(enable_seek_to_import);

  QCheckBox* enable_audio_scrubbing = new QCheckBox(tr("Audio Scrubbing"));
  AddBoolPair(enable_audio_scrubbing, &hitvidio::CurrentConfig.enable_audio_scrubbing);
  ...
  AddBoolPair(enable_drop_on_media_to_replace, &hitvidio::CurrentConfig.drop_on_media_to_replace);
  behavior_tab_layout->Add(enable_drop_on_media_to_replace);

  QCheckBox* enable_hover_focus = new QCheckBox(tr("Enable Hover Focus"));
  AddBoolPair(enable_hover_focus, &hitvidio::CurrentConfig.hover_focus);
  ...
  AddBoolPair(set_name_and_marker, &hitvidio::CurrentConfig.set_name_with_marker);
  ...
  ui_style->addItem(tr("Hitvidio Dark (Default)"), hitvidio::styling::kHitvidioDefaultDark);
  ui_style->addItem(tr("Hitvidio Light"), hitvidio::styling::kHitvidioDefaultLight);
  ui_style->addItem(tr("Native"), hitvidio::styling::kNativeDarkIcons);
  ui_style->addItem(tr("Native (Light Icons)"), hitvidio::styling::kNativeLightIcons);
  ui_style->setCurrentIndex(hitvidio::CurrentConfig.style);
  ...
  AddBoolPair(native_menus, &hitvidio::CurrentConfig.use_native_menu_styling, true);
  ...
  custom_css_fn->setText(hitvidio::CurrentConfig.css_path);
  ...
  effect_textbox_lines_field->setValue(hitvidio::CurrentConfig.effect_textbox_lines);
  ...
  upcoming_queue_spinbox->setValue(hitvidio::CurrentConfig.upcoming_queue_size);
  ...
  upcoming_queue_type->setCurrentIndex(hitvidio::CurrentConfig.upcoming_queue_type);
  ...
  previous_queue_spinbox->setValue(hitvidio::CurrentConfig.previous_queue_size);
  ...
  previous_queue_type->setCurrentIndex(hitvidio::CurrentConfig.previous_queue_type);
  ...
  for (int i=0;i<devs.size();i++) {
    ...
    if (!found_preferred_device
        && devs.at(i).deviceName() == hitvidio::CurrentConfig.preferred_audio_output) {
  ...
  for (int i=0;i<devs.size();i++) {
    ...
    if (!found_preferred_device
        && devs.at(i).deviceName() == hitvidio::CurrentConfig.preferred_audio_input) {
  ...
  for (int i=0;i<audio_sample_rate->count();i++) {
    if (audio_sample_rate->itemData(i).toInt() == hitvidio::CurrentConfig.audio_rate) {
  ...
  recordingComboBox->setCurrentIndex(hitvidio::CurrentConfig.recording_mode - 1);
