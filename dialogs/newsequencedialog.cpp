...
void NewSequenceDialog::setup_ui() {
  ...
  width_numeric->setValue(hitvidio::CurrentConfig.default_sequence_width);
  ...
  height_numeric->setValue(hitvidio::CurrentConfig.default_sequence_height);
  ...
  for (int i=0;i<frame_rate_combobox->count();i++) {
    if (qFuzzyCompare(frame_rate_combobox->itemData(i).toDouble(), hitvidio::CurrentConfig.default_sequence_framerate)) {
      frame_rate_combobox->setCurrentIndex(i);
    }
  }
  ...
  for (int i=0;i<audio_frequency_combobox->count();i++) {
    if (audio_frequency_combobox->itemData(i) == hitvidio::CurrentConfig.default_sequence_audio_frequency) {
      audio_frequency_combobox->setCurrentIndex(i);
    }
  }
